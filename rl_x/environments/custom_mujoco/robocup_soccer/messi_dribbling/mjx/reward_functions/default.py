...
class DefaultReward:
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        # Ball chasing reward
        ball_com_distance_squared = jnp.sum(jnp.square(ball_pos_world - com_pos_world))
        chasing_ball_raw = jnp.exp(-2.0 * ball_com_distance_squared)
        ...
        feet_floor_contacts = self.env.terrain_function.check_feet_floor_contact(data)
        ball_distance_xy = jnp.linalg.norm(ball_pos_world[:2] - base_pos_world[:2])
        is_standing_command = jnp.all(ball_velocity_command_xy == 0.0) & (ball_distance_xy < 0.5)
        ...
        info[f"reward/ball_velocity_tracking"] = projected_ball_velocity_reward
        ...
        info[f"env_info/ball_distance_to_base"] = jnp.linalg.norm(ball_pos_world[:2] - base_pos_world[:2])
        info[f"env_info/ball_distance_to_com"] = jnp.linalg.norm(ball_pos_world - com_pos_world)
        ...
        info[f"env_info/base_yaw"] = base_yaw
        info[f"env_info/base_yaw_rate"] = base_yaw_rate
        info[f"env_info/ball_velocity_command_x"] = ball_velocity_command_xy[0]
        info[f"env_info/ball_velocity_command_y"] = ball_velocity_command_xy[1]
