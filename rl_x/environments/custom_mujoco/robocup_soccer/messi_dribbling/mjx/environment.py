...
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.state import State
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mjx.terrain_functions.handler import get_terrain_function


class MessiDribblingEnv:
    ...
    def update_ball_sensing(self, data, internal_state, info, reset_timer):
        ...
        time_since_ball_seen = jnp.where(
            reset_or_visible,
            0.0,
            internal_state["time_since_ball_seen"] + self.dt,
        )
        ...
        internal_state["ball_detection_distance"] = jnp.where(ball_visible, distance, 0.0)
        internal_state["ball_detection_azimuth"] = jnp.where(ball_visible, azimuth, 0.0)
        internal_state["ball_detection_elevation"] = jnp.where(ball_visible, elevation, 0.0)
        internal_state["ball_detection_local_pos"] = jnp.where(ball_visible, local_pos, jnp.zeros(3))
    ...
    def render(self, state):
        ...
        if self.runner_mode == "test":
            ...
            if explicit_velocity_commands:
                ball_velocity_command = jnp.array([goal_x_velocity, goal_y_velocity])
                ball_velocity_command = jnp.where(jnp.abs(ball_velocity_command) < (self.command_function.zero_clip_threshold_percentage * state.internal_state["max_ball_velocity"]), 0.0, ball_velocity_command)
                ball_velocity_command = jnp.clip(ball_velocity_command, -state.internal_state["max_ball_velocity"], state.internal_state["max_ball_velocity"])
                state.internal_state["ball_velocity_command"] = jnp.tile(ball_velocity_command, (self.nr_envs, 1))
                actuator_joint_keep_nominal = jnp.where(jnp.all(ball_velocity_command == 0.0), jnp.ones(self.nr_actuator_joints, dtype=bool), self.command_function.default_actuator_joint_keep_nominal)
        ...
        if self.add_goal_arrow:
            ball_velocity_command = state.internal_state["ball_velocity_command"][env_id]
    ...
    def _step(self, state, action):
        ...
        terminated = self.termination_function.should_terminate(state.internal_state) | jnp.any(jnp.abs(data.qvel[:3]) == 100.0) | state.internal_state["ball_unseen_too_long"]
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        ...
        relative_ball_position_base = self.relative_ball_position_base(data, internal_state)
