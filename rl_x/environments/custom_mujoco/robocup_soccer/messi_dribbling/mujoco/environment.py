...
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.messi_dribbling.mujoco.terrain_functions.handler import get_terrain_function


class MessiDribblingEnv(gym.Env):
    ...
    def relative_ball_position_base(self):
        ...
        ball_rel_base_xy = self.rotate_world_to_base_xy(ball_pos[:2] - base_pos[:2], self.internal_state["imu_orientation_euler"][2])
    ...
    def update_ball_sensing(self, reset_timer):
        ...
        self.internal_state["ball_detection_distance"] = np.where(ball_visible, distance, 0.0)
        self.internal_state["ball_detection_azimuth"] = np.where(ball_visible, azimuth, 0.0)
        self.internal_state["ball_detection_elevation"] = np.where(ball_visible, elevation, 0.0)
        self.internal_state["ball_detection_local_pos"] = np.where(ball_visible, local_pos, np.zeros(3))
    ...
    def step(self, action):
        ...
        terminated = self.termination_function.should_terminate() | np.any(np.abs(self.internal_state["data"].qvel[:3]) == 100.0) | self.internal_state["ball_unseen_too_long"]
    ...
    def get_observation(self, action):
        ...
        relative_ball_position_base = self.relative_ball_position_base()
