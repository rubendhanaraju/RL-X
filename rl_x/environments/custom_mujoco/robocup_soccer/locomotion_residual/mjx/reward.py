...
class ResidualRunReward:
    def __init__(self, env):
        ...
        self.forward_speed_coeff = env.env_config["reward"]["forward_speed_coeff"] * env.dt
        self.forward_speed_clip = env.env_config["reward"]["forward_speed_clip"]
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        curriculum_coeff = jnp.asarray(1.0, dtype=jnp.float32)
        
        # Tracking xy command direction without tracking speed magnitude.
        ...
        current_imu_linear_velocity_xy = current_imu_linear_velocity[:2]
        desired_xy_speed = jnp.linalg.norm(desired_imu_linear_velocity_xy)
        current_xy_speed = jnp.linalg.norm(current_imu_linear_velocity_xy)
        desired_xy_direction = desired_imu_linear_velocity_xy / jnp.maximum(desired_xy_speed, 1e-6)
        current_xy_direction = current_imu_linear_velocity_xy / jnp.maximum(current_xy_speed, 1e-6)
        xy_direction_difference = desired_xy_direction - current_xy_direction
        xy_velocity_difference_norm = jnp.sum(jnp.square(xy_direction_difference))
        has_xy_command = desired_xy_speed > (
            self.env.command_function.zero_clip_threshold_percentage
            * internal_state["max_command_velocity"]
        )
        tracking_xy_velocity_command_reward = jnp.where(
            has_xy_command,
        ...
        # Positive forward velocity reward for residual run training.
        forward_speed = jnp.clip(current_imu_linear_velocity[0], 0.0, self.forward_speed_clip)
        forward_speed_reward = self.forward_speed_coeff * forward_speed
        ...
        # Total reward
        tracking_reward = tracking_xy_velocity_command_reward + tracking_yaw_velocity_command_reward + forward_speed_reward + feet_phase_reward
        ...
        info[f"reward/forward_speed"] = forward_speed_reward
        ...
        info[f"env_info/forward_speed"] = forward_speed
        info[f"env_info/xy_command_direction_error"] = jnp.nan_to_num(
            jnp.mean(jnp.abs(xy_direction_difference)),
            nan=1.0,
            posinf=1.0,
            neginf=1.0,
        )
        info[f"env_info/xy_vel_diff_abs"] = info[f"env_info/xy_command_direction_error"]
