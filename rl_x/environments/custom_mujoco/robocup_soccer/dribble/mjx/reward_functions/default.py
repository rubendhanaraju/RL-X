...
class DefaultReward:
    def __init__(self, env):
        ...
        self.tracking_xy_velocity_command_coeff = self.get_reward_config_value("tracking_xy_velocity_command_coeff") * env.dt
        self.tracking_xy_temperature = env.env_config["reward"]["tracking_xy_temperature"]
        self.tracking_yaw_velocity_command_coeff = self.get_reward_config_value("tracking_yaw_velocity_command_coeff") * env.dt
        self.tracking_yaw_temperature = env.env_config["reward"]["tracking_yaw_temperature"]
        self.chasing_coeff = self.get_reward_config_value("chasing_coeff") * env.dt
        self.chasing_distance_coeff = env.env_config["reward"]["chasing_distance_coeff"]
        self.projected_ball_velocity_coeff = self.get_reward_config_value("projected_ball_velocity_coeff") * env.dt
        self.projected_ball_velocity_temperature = env.env_config["reward"]["projected_ball_velocity_temperature"]
        self.yaw_alignment_coeff = self.get_reward_config_value("yaw_alignment_coeff") * env.dt
        self.yaw_alignment_error_coeff = env.env_config["reward"]["yaw_alignment_error_coeff"]
        ...
        self.actuator_joint_nominal_diff_coeff = self.get_reward_config_value("actuator_joint_nominal_diff_coeff") * env.dt
    ...
    def get_reward_config_value(self, name):
        return self.env.dribble_curriculum_stage_config.get(name, self.env.env_config["reward"][name])
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        # Chasing reward: keep the robot close enough to actively play the ball.
        ball_position_xy = data.qpos[self.env.ball_qpos_adr:self.env.ball_qpos_adr + 2]
        base_position_xy = data.qpos[:2]
        ball_to_base_xy = ball_position_xy - base_position_xy
        ball_distance_squared = jnp.sum(jnp.square(ball_to_base_xy))
        chasing_reward = self.chasing_coeff * jnp.exp(-self.chasing_distance_coeff * ball_distance_squared)

        # Projected ball velocity reward: track commanded ball velocity along the commanded direction.
        desired_ball_velocity_xy = internal_state["goal_velocities"][:2]
        ball_velocity_xy = data.qvel[self.env.ball_qvel_adr:self.env.ball_qvel_adr + 2]
        desired_ball_speed = jnp.linalg.norm(desired_ball_velocity_xy)
        desired_ball_direction = desired_ball_velocity_xy / jnp.maximum(desired_ball_speed, 1e-6)
        projected_ball_velocity_xy = jnp.dot(ball_velocity_xy, desired_ball_direction) * desired_ball_direction
        ball_velocity_for_tracking = jnp.where(desired_ball_speed > 1e-6, projected_ball_velocity_xy, ball_velocity_xy)
        ball_velocity_difference = desired_ball_velocity_xy - ball_velocity_for_tracking
        ball_velocity_difference_norm = jnp.sum(jnp.square(ball_velocity_difference))
        projected_ball_velocity_reward = self.projected_ball_velocity_coeff * jnp.exp(-ball_velocity_difference_norm / self.projected_ball_velocity_temperature)

        # Yaw alignment reward: face the commanded ball direction while keeping the ball in front.
        base_yaw = self.env._root_yaw_from_qpos(data.qpos)
        desired_ball_velocity_yaw = jnp.where(
            desired_ball_speed > 1e-6,
            jnp.arctan2(desired_ball_velocity_xy[1], desired_ball_velocity_xy[0]),
            base_yaw,
        )
        ball_yaw = jnp.arctan2(ball_to_base_xy[1], ball_to_base_xy[0])
        yaw_to_command_error = self.wrap_to_pi(base_yaw - desired_ball_velocity_yaw)
        yaw_to_ball_error = self.wrap_to_pi(base_yaw - ball_yaw)
        yaw_alignment_error = jnp.square(yaw_to_command_error) + jnp.square(yaw_to_ball_error)
        yaw_alignment_reward = self.yaw_alignment_coeff * jnp.exp(-self.yaw_alignment_error_coeff * yaw_alignment_error)
        ...
        # Total reward
        tracking_reward = tracking_xy_velocity_command_reward + tracking_yaw_velocity_command_reward + feet_phase_reward + \
                          chasing_reward + projected_ball_velocity_reward + yaw_alignment_reward
        ...
        info[f"reward/chasing"] = chasing_reward
        info[f"reward/projected_ball_velocity"] = projected_ball_velocity_reward
