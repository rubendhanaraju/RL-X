...
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.state import State
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.dribble.mjx.terrain_functions.handler import get_terrain_function
...
class DribbleEnv:
    def __init__(self, robot_config, runner_mode, render, env_config, nr_envs):
        ...
        self.dribble_config = env_config["dribble"]
        self.dribble_curriculum_stage = int(env_config["dribble_curriculum"]["stage"]) if "dribble_curriculum" in env_config else 0
        self.dribble_curriculum_stage_config = env_config["dribble_curriculum"][f"stage_{self.dribble_curriculum_stage}"] if self.dribble_curriculum_stage else {}
        legacy_env_curriculum_config = env_config["legacy_env_curriculum"] if "legacy_env_curriculum" in env_config else {}
        self.update_env_curriculum_from_episode_return = bool(
            self.dribble_curriculum_stage_config.get(
                "legacy_env_curriculum_enabled",
                legacy_env_curriculum_config.get("enabled", True),
            )
        )
        self.fixed_env_curriculum_coeff = float(
            self.dribble_curriculum_stage_config.get(
                "legacy_env_curriculum_fixed_coeff",
                legacy_env_curriculum_config.get("fixed_coeff", 1.0),
            )
        )
        ...
        self.ball_reset_x_range = jnp.array(self.get_dribble_config_value("ball_reset_x_range"), dtype=jnp.float32)
        self.ball_reset_y_range = jnp.array(self.get_dribble_config_value("ball_reset_y_range"), dtype=jnp.float32)
        self.ball_position_observation_scale = jnp.float32(self.get_dribble_config_value("ball_position_observation_scale"))
        self.ball_velocity_observation_scale = jnp.float32(self.get_dribble_config_value("ball_velocity_observation_scale"))
        self.ball_termination_distance = jnp.float32(
            self.dribble_curriculum_stage_config.get(
                "ball_distance_threshold",
                env_config["termination"]["ball_distance_threshold"],
            )
        )
    ...
    def get_dribble_config_value(self, name):
        return self.dribble_curriculum_stage_config.get(name, self.dribble_config[name])


    @staticmethod
    def _add_ball_to_world(xml_handle, ball_config):
        worldbody = xml_handle.worldbody
        ball_body = worldbody.add("body", name="ball", pos=f"0 0 {ball_config['radius']}")
    ...
    def _enable_ball_robot_contacts(xml_handle):
        ...
        for geom in xml_handle.find_all("geom"):
            ...
            if geom.name in (None, floor_name, ball_name):
                continue
            if geom_class == "reward_collision_sphere":
                continue
            if not is_foot_geom and geom_class != "foot":
                continue
    ...
    @staticmethod
    def _root_yaw_from_qpos(qpos):
        w, x, y, z = qpos[3:7]
        return jnp.arctan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))


    @staticmethod
    def _rotate_world_to_base_xy(vector_xy, base_yaw):
    ...
    @staticmethod
    def _rotate_base_to_world_xy(vector_xy, base_yaw):
        cos_yaw = jnp.cos(base_yaw)
        sin_yaw = jnp.sin(base_yaw)
        return jnp.array([
            cos_yaw * vector_xy[0] - sin_yaw * vector_xy[1],
    ...
    def _get_ball_state_for_observation(self, data):
        base_yaw = self._root_yaw_from_qpos(data.qpos)
        ball_position_xy = data.qpos[self.ball_qpos_adr:self.ball_qpos_adr + 2]
        base_position_xy = data.qpos[:2]
        ball_velocity_xy = data.qvel[self.ball_qvel_adr:self.ball_qvel_adr + 2]
        return (
            self._rotate_world_to_base_xy(ball_position_xy - base_position_xy, base_yaw),
            self._rotate_world_to_base_xy(ball_velocity_xy, base_yaw),
        )


    def _sample_ball_reset_state(self, qpos, qvel, internal_state, key):
        ball_x_key, ball_y_key = jax.random.split(key)
        ball_offset_base_xy = jnp.array([
            jax.random.uniform(ball_x_key, minval=self.ball_reset_x_range[0], maxval=self.ball_reset_x_range[1]),
            jax.random.uniform(ball_y_key, minval=self.ball_reset_y_range[0], maxval=self.ball_reset_y_range[1]),
        ], dtype=jnp.float32)

        base_yaw = self._root_yaw_from_qpos(qpos)
        ball_position_xy = qpos[:2] + self._rotate_base_to_world_xy(ball_offset_base_xy, base_yaw)
        ball_height = self.terrain_function.ground_height_at(internal_state, ball_position_xy[0], ball_position_xy[1]) + self.ball_radius

        qpos = qpos.at[self.ball_qpos_adr:self.ball_qpos_adr + 3].set(jnp.array([ball_position_xy[0], ball_position_xy[1], ball_height], dtype=jnp.float32))
        qpos = qpos.at[self.ball_qpos_adr + 3:self.ball_qpos_adr + 7].set(jnp.array([1.0, 0.0, 0.0, 0.0], dtype=jnp.float32))
        qvel = qvel.at[self.ball_qvel_adr:self.ball_qvel_adr + 6].set(jnp.zeros(6, dtype=jnp.float32))
    ...
    def reset(self, key, eval_mode):
        ...
        internal_state = {
            ...
            "env_curriculum_coeff": jnp.where(
                self.update_env_curriculum_from_episode_return,
                jnp.where(eval_mode, 1.0, 0.0),
                jnp.float32(self.fixed_env_curriculum_coeff),
            ),
    ...
    def _reset(self, state):
        key, initial_state_key, terrain_key, domain_randomization_key, observation_key, gait_manager_key, ball_reset_key = jax.random.split(state.key, 7)
        ...
        qpos, qvel = self._sample_ball_reset_state(qpos, qvel, state.internal_state, ball_reset_key)
        data = data.replace(qpos=qpos, qvel=qvel, ctrl=jnp.zeros(self.nr_actuator_joints))
        # data = mjx.forward(self.initial_mjx_model, data)

        new_state = state

        if self.update_env_curriculum_from_episode_return:
            ...
            new_state.internal_state["env_curriculum_coeff"] = jnp.float32(self.fixed_env_curriculum_coeff)
    ...
    def _step(self, state, action):
        ...
        ball_distance = jnp.linalg.norm(data.qpos[self.ball_qpos_adr:self.ball_qpos_adr + 2] - data.qpos[:2])
        ball_too_far = (self.ball_termination_distance > 0.0) & (ball_distance > self.ball_termination_distance)
        terminated = self.termination_function.should_terminate(state.internal_state) | jnp.any(jnp.abs(data.qvel[:3]) == 100.0) | ball_too_far
        ...
        state.info["env_info/ball_distance"] = ball_distance
        state.info["env_info/ball_too_far"] = ball_too_far
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        base_yaw = self._root_yaw_from_qpos(data.qpos)
        goal_velocity_base_xy = self._rotate_world_to_base_xy(internal_state["goal_velocities"][:2], base_yaw)
        observed_goal_velocities = jnp.concatenate([goal_velocity_base_xy, internal_state["goal_velocities"][2:3]])
        ball_position_base_xy, ball_velocity_base_xy = self._get_ball_state_for_observation(data)
        observation = jnp.concatenate([
            ...
            ball_position_base_xy,
            ball_velocity_base_xy,
        ])
        ...
        observation = observation.at[self.ball_position_obs_idx].set(jnp.clip(observation[self.ball_position_obs_idx] / self.ball_position_observation_scale, -1.0, 1.0))
        observation = observation.at[self.ball_velocity_obs_idx].set(jnp.clip(observation[self.ball_velocity_obs_idx] / self.ball_velocity_observation_scale, -1.0, 1.0))
    ...
    def get_observation_space(self):
        ...
        self.ball_position_obs_idx = jnp.array([current_observation_idx + i for i in range(2)])
        current_observation_idx += 2
        self.ball_velocity_obs_idx = jnp.array([current_observation_idx + i for i in range(2)])
        current_observation_idx += 2
        ...
        self.policy_observation_indices = jnp.concatenate([
            ...
            self.ball_position_obs_idx,
            self.ball_velocity_obs_idx,
        ], dtype=int)
