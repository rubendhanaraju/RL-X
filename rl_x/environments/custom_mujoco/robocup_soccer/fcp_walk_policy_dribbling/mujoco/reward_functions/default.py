...
class DefaultReward:
    def __init__(self, env):
        ...
        self.fcp_dribble_gate_by_possession = reward_config["fcp_dribble_gate_by_possession"]
        self.ball_possession_coeff = reward_config["ball_possession_coeff"]
        self.ball_possession_target = np.array(
            [
                reward_config["ball_possession_target_x"],
                reward_config["ball_possession_target_y"],
            ],
            dtype=np.float32,
        )
        self.ball_possession_deadzone = np.array(
            [
                reward_config["ball_possession_deadzone_x"],
                reward_config["ball_possession_deadzone_y"],
            ],
            dtype=np.float32,
        )
    ...
    def reward_and_info(self, action):
        ...
        desired_dribble_direction = np.where(
            ...
            ball_velocity_command_xy / np.maximum(desired_ball_speed, 1e-6),
            base_forward_world,
        )
        fcp_dribble_forward_velocity = np.dot(ball_vel_world[:2], desired_dribble_direction)
        fcp_dribble_lateral_velocity = np.linalg.norm(
            ball_vel_world[:2] - fcp_dribble_forward_velocity * desired_dribble_direction
        )
        fcp_dribble_forward_velocity_clipped = np.clip(
        ...
        ball_rel_base_for_possession = self.env.relative_ball_position_base()
        ball_possession_error = np.maximum(
            np.abs(ball_rel_base_for_possession[:2] - self.ball_possession_target)
            - self.ball_possession_deadzone,
            0.0,
        )
        ball_possession_error_normalized = ball_possession_error / np.maximum(self.ball_possession_scale, 1e-6)
        ...
        feet_floor_contacts = self.env.terrain_function.check_feet_floor_contact()
        ball_distance_xy = np.linalg.norm(ball_pos_world[:2] - base_pos_world[:2])
        is_standing_command = np.all(ball_velocity_command_xy == 0.0) & (ball_distance_xy < 0.5)
        ...
        self.env.internal_state["info"][f"env_info/ball_distance_to_base"] = np.linalg.norm(ball_pos_world[:2] - base_pos_world[:2])
        self.env.internal_state["info"][f"env_info/ball_distance_to_com"] = np.linalg.norm(ball_pos_world - com_pos_world)
        self.env.internal_state["info"][f"env_info/ball_speed"] = np.linalg.norm(ball_vel_world[:2])
        self.env.internal_state["info"][f"env_info/fcp_dribble_raw_reward"] = fcp_dribble_raw_reward
        self.env.internal_state["info"][f"env_info/fcp_dribble_forward_velocity"] = fcp_dribble_forward_velocity
        self.env.internal_state["info"][f"env_info/fcp_dribble_forward_velocity_clipped"] = fcp_dribble_forward_velocity_clipped
        self.env.internal_state["info"][f"env_info/fcp_dribble_lateral_velocity"] = fcp_dribble_lateral_velocity
        self.env.internal_state["info"][f"env_info/fcp_dribble_lateral_penalty"] = fcp_dribble_lateral_penalty
        self.env.internal_state["info"][f"env_info/fcp_dribble_possession_gate"] = fcp_dribble_possession_gate
        self.env.internal_state["info"][f"env_info/ball_possession_error_x"] = ball_possession_error[0]
        self.env.internal_state["info"][f"env_info/ball_possession_error_y"] = ball_possession_error[1]
        self.env.internal_state["info"][f"env_info/ball_possession_penalty_norm"] = ball_possession_penalty_norm
        self.env.internal_state["info"][f"env_info/ball_velocity_command_norm"] = desired_ball_speed
        self.env.internal_state["info"][f"env_info/ball_velocity_tracking_error"] = np.sqrt(ball_velocity_tracking_error)
        self.env.internal_state["info"][f"env_info/projected_ball_velocity_tracking_error"] = np.sqrt(ball_velocity_tracking_error)
        self.env.internal_state["info"][f"env_info/base_yaw"] = base_yaw
        self.env.internal_state["info"][f"env_info/base_yaw_rate"] = base_yaw_rate
        self.env.internal_state["info"][f"env_info/ball_velocity_command_x"] = ball_velocity_command_xy[0]
        self.env.internal_state["info"][f"env_info/ball_velocity_command_y"] = ball_velocity_command_xy[1]
        self.env.internal_state["info"][f"env_info/xy_vel_diff_abs"] = 0.0
