...
class DefaultDRSeenRobotFunction:
    ...
    def sample(self):
        ...
        # Visualization-only bodies, like the optional goal arrow, can have zero mass.
        # Keep their inertia scaling neutral so render-only helpers cannot inject NaNs.
        mass_ratio = np.divide(
            coupled_masses,
            default_masses,
            out=np.ones_like(coupled_masses),
            where=np.abs(default_masses) > 1e-12,
        )
        coupled_inertias = default_inertias * mass_ratio.reshape(-1, 1)
        ...
        if hasattr(self.env, "ball_body_id"):
            ball_body_idx = self.env.ball_body_id - 1
            ball_geom_idx = self.env.ball_geom_id - 1
            ball_joint_idx = self.env.ball_joint_id - 1
            ball_dof_start = self.env.ball_qveladr - 6
            ball_dof_slice = slice(ball_dof_start, ball_dof_start + 6)

            masses[ball_body_idx] = self.env.initial_mj_model.body_mass[self.env.ball_body_id]
            inertias[ball_body_idx] = self.env.initial_mj_model.body_inertia[self.env.ball_body_id]
            coms[ball_body_idx] = self.env.initial_mj_model.body_ipos[self.env.ball_body_id]
            inertia_quats[ball_body_idx] = self.env.initial_mj_model.body_iquat[self.env.ball_body_id]
            body_positions[ball_body_idx] = self.env.initial_mj_model.body_pos[self.env.ball_body_id]
            body_quats[ball_body_idx] = self.env.initial_mj_model.body_quat[self.env.ball_body_id]

            geom_positions[ball_geom_idx] = self.env.initial_mj_model.geom_pos[self.env.ball_geom_id]
            geom_sizes[ball_geom_idx] = self.env.initial_mj_model.geom_size[self.env.ball_geom_id]
            geom_rbounds[ball_geom_idx] = self.env.initial_mj_model.geom_rbound[self.env.ball_geom_id]
            geom_aabbs[ball_geom_idx] = self.env.initial_mj_model.geom_aabb[self.env.ball_geom_id]

            joint_positions[ball_joint_idx] = self.env.initial_mj_model.jnt_pos[self.env.ball_joint_id]
            rotated_joint_rotation_axes[ball_joint_idx] = self.env.initial_mj_model.jnt_axis[self.env.ball_joint_id]
            joint_ranges[ball_joint_idx] = self.env.initial_mj_model.jnt_range[self.env.ball_joint_id]
            jnt_stiffness[ball_joint_idx] = self.env.initial_mj_model.jnt_stiffness[self.env.ball_joint_id]

            dof_damping[ball_dof_slice] = self.env.initial_mj_model.dof_damping[self.env.ball_qveladr:self.env.ball_qveladr + 6]
            dof_armature[ball_dof_slice] = self.env.initial_mj_model.dof_armature[self.env.ball_qveladr:self.env.ball_qveladr + 6]
            dof_frictionloss[ball_dof_slice] = self.env.initial_mj_model.dof_frictionloss[self.env.ball_qveladr:self.env.ball_qveladr + 6]
            site_positions[self.env.ball_site_id] = self.env.initial_mj_model.site_pos[self.env.ball_site_id]
