...
class DefaultDRSeenRobotFunction:
    ...
    def sample(self, internal_state, mjx_model, data, should_randomize, key):
        ...
        if hasattr(self.env, "ball_body_id"):
            ball_body_id = self.env.ball_body_id
            ball_geom_id = self.env.ball_geom_id
            ball_joint_id = self.env.ball_joint_id
            ball_qveladr = self.env.ball_qveladr

            body_positions = body_positions.at[ball_body_id].set(self.env.initial_mjx_model.body_pos[ball_body_id])
            body_quats = body_quats.at[ball_body_id].set(self.env.initial_mjx_model.body_quat[ball_body_id])
            body_mass = body_mass.at[ball_body_id].set(self.env.initial_mjx_model.body_mass[ball_body_id])
            body_inertia = body_inertia.at[ball_body_id].set(self.env.initial_mjx_model.body_inertia[ball_body_id])
            body_ipos = body_ipos.at[ball_body_id].set(self.env.initial_mjx_model.body_ipos[ball_body_id])
            body_iquat = body_iquat.at[ball_body_id].set(self.env.initial_mjx_model.body_iquat[ball_body_id])

            geom_positions = geom_positions.at[ball_geom_id].set(self.env.initial_mjx_model.geom_pos[ball_geom_id])
            geom_sizes = geom_sizes.at[ball_geom_id].set(self.env.initial_mjx_model.geom_size[ball_geom_id])
            geom_rbounds = geom_rbounds.at[ball_geom_id].set(self.env.initial_mjx_model.geom_rbound[ball_geom_id])

            joint_positions = joint_positions.at[ball_joint_id].set(self.env.initial_mjx_model.jnt_pos[ball_joint_id])
            jnt_axis = jnt_axis.at[ball_joint_id].set(self.env.initial_mjx_model.jnt_axis[ball_joint_id])
            jnt_range = jnt_range.at[ball_joint_id].set(self.env.initial_mjx_model.jnt_range[ball_joint_id])
            jnt_stiffness = jnt_stiffness.at[ball_joint_id].set(self.env.initial_mjx_model.jnt_stiffness[ball_joint_id])

            ball_dof_slice = slice(ball_qveladr, ball_qveladr + 6)
            dof_damping = dof_damping.at[ball_dof_slice].set(self.env.initial_mjx_model.dof_damping[ball_dof_slice])
            dof_armature = dof_armature.at[ball_dof_slice].set(self.env.initial_mjx_model.dof_armature[ball_dof_slice])
            dof_frictionloss = dof_frictionloss.at[ball_dof_slice].set(self.env.initial_mjx_model.dof_frictionloss[ball_dof_slice])
