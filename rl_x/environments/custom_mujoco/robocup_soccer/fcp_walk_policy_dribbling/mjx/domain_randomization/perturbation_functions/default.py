...
class DefaultDRPerturbation:
    ...
    def sample(self, internal_state, mjx_model, data, should_randomize, key):
        ...
        actuator_joint_velocity = (
            data.qvel[self.env.actuator_joint_mask_qvel]
            + internal_state["env_curriculum_coeff"]
            * jax.random.uniform(
                joint_velocity_key,
                data.qvel[self.env.actuator_joint_mask_qvel].shape,
                minval=-self.max_joint_velocity,
                maxval=self.max_joint_velocity,
            )
        )
        actuator_joint_velocity = jnp.where(
            should_randomize,
            actuator_joint_velocity,
            data.qvel[self.env.actuator_joint_mask_qvel],
        )
        actuator_joint_position = (
            data.qpos[self.env.actuator_joint_mask_qpos]
            + internal_state["env_curriculum_coeff"]
            * jax.random.uniform(
                joint_position_key,
                data.qpos[self.env.actuator_joint_mask_qpos].shape,
                minval=-self.max_joint_position,
                maxval=self.max_joint_position,
            )
        )
        actuator_joint_position = jnp.where(
            should_randomize,
            actuator_joint_position,
            data.qpos[self.env.actuator_joint_mask_qpos],
        )
        ...
        data = data.replace(
            qpos=data.qpos.at[self.env.actuator_joint_mask_qpos].set(actuator_joint_position),
            qvel=data.qvel.at[:6].set(trunk_velocity).at[self.env.actuator_joint_mask_qvel].set(actuator_joint_velocity),
        )
