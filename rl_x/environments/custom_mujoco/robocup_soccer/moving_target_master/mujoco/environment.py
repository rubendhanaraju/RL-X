...
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mujoco.terrain_functions.handler import get_terrain_function
...
class MovingTargetMasterEnv(gym.Env):
    def __init__(self, robot_config, runner_mode, seed, render, env_config, nr_envs):
        ...
        self.internal_state = {
            ...
            "target_velocity_world": np.array([0.0, 0.0]),
    ...
    def sample_target_velocity(self):
        target_direction = self.np_rng.uniform(low=-np.pi, high=np.pi)
        target_speed = self.np_rng.uniform(low=self.target_speed_min, high=self.target_speed_max)
        return target_speed * np.array([np.cos(target_direction), np.sin(target_direction)])
    ...
    def sample_point_reset(self, qpos, qvel):
        ...
        if self.spawn_target_in_heading_cone:
            relative_angle = self.np_rng.uniform(low=-self.target_spawn_half_angle, high=self.target_spawn_half_angle)
        ...
        target_velocity = self.sample_target_velocity()

        qpos[self.point_qposadr:self.point_qposadr + 7] = point_qpos
        qvel[self.point_qveladr:self.point_qveladr + 6] = np.concatenate([target_velocity, np.zeros(4)])
        self.internal_state["target_velocity_world"] = target_velocity

        return qpos, qvel


    def advance_target(self, point_qpos):
        next_point_qpos = point_qpos.copy()
        next_point_qpos[:2] = next_point_qpos[:2] + self.internal_state["target_velocity_world"] * self.dt
        next_point_qpos[2] = self.terrain_function.ground_height_at(next_point_qpos[0], next_point_qpos[1]) + self.point_radius
        return next_point_qpos
    ...
    def update_point_sensing(self, reset_timer):
        target_xy_distance_to_base = np.linalg.norm(
            self.point_position_world()[:2] - self.base_position_world()[:2]
        )
        self.internal_state["info"]["env_info/target_xy_distance_to_base"] = target_xy_distance_to_base
        self.internal_state["info"]["env_info/target_xy_distance_to_com"] = target_xy_distance_to_base
        self.internal_state["info"]["env_info/point_xy_distance_to_base"] = target_xy_distance_to_base
        self.internal_state["info"]["env_info/point_xy_distance_to_com"] = target_xy_distance_to_base
    ...
    def render(self):
        ...
        if self.add_target_velocity_arrow:
            target_velocity = self.internal_state["target_velocity_world"]
    ...
    def step(self, action):
        ...
        target_qpos = self.advance_target(point_qpos)
        target_qvel = np.concatenate([self.internal_state["target_velocity_world"], np.zeros(4)])
        self.internal_state["data"].qpos[self.point_qposadr:self.point_qposadr + 7] = target_qpos
        self.internal_state["data"].qvel[self.point_qveladr:self.point_qveladr + 6] = target_qvel
        mujoco.mj_forward(self.internal_state["mj_model"], self.internal_state["data"])
    ...
    def get_observation(self, action):
        ...
        observation[self.relative_point_position_obs_idx] = np.clip(observation[self.relative_point_position_obs_idx] / self.point_spawn_radius, -1.0, 1.0)
        ...
        observation[self.point_velocity_world_obs_idx] = np.clip(observation[self.point_velocity_world_obs_idx] / self.target_speed_max, -1.0, 1.0)
    ...
    def get_observation_space(self):
        ...
        self.relative_point_position_obs_idx = np.array([current_observation_idx + i for i in range(3)], dtype=int)
        current_observation_idx += 3
