...
class DefaultReward:
    def __init__(self, env):
        ...
        self.follow_coeff = reward_config["follow_coeff"] * env.dt
    ...
    def reward_and_info(self, action):
        target_pos_world = self.env.point_position_world()
        target_vel_world = self.env.point_velocity_world()
        base_pos_world = self.env.base_position_world()
        base_yaw = self.env.internal_state["imu_orientation_euler"][2]

        current_imu_angular_velocity = self.env.internal_state["data"].sensordata[
            self.env.imu_angular_velocity_sensor_adr:self.env.imu_angular_velocity_sensor_adr + self.env.imu_angular_velocity_sensor_dim
        ]
        ...
        follow_distance = np.linalg.norm(base_pos_world[:2] - target_pos_world[:2])
        follow_reward_raw = np.exp(-0.5 * follow_distance)
        follow_reward = self.follow_coeff * follow_reward_raw
        ...
        reward = base_orientation_reward + feet_orientation_reward + feet_distance_reward + feet_clearance_reward + termination_reward + \
                 ...
                 collision_reward + follow_reward
        ...
        self.env.internal_state["info"]["reward/follow"] = follow_reward
        self.env.internal_state["info"]["env_info/follow_reward_raw"] = follow_reward_raw
        self.env.internal_state["info"]["env_info/follow_distance"] = follow_distance
        self.env.internal_state["info"]["reward/total"] = reward
        self.env.internal_state["info"]["env_info/target_xy_distance_to_base"] = follow_distance
        self.env.internal_state["info"]["env_info/target_distance_to_base"] = np.linalg.norm(target_pos_world - base_pos_world)
        self.env.internal_state["info"]["env_info/target_speed"] = np.linalg.norm(target_vel_world[:2])
        self.env.internal_state["info"]["env_info/target_velocity_x"] = target_vel_world[0]
        self.env.internal_state["info"]["env_info/target_velocity_y"] = target_vel_world[1]
