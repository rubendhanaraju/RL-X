...
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.state import State
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.moving_target_master.mjx.terrain_functions.handler import get_terrain_function
...
class MovingTargetMasterEnv:
    def __init__(self, robot_config, runner_mode, render, env_config, nr_envs):
        ...
        self.add_target_velocity_arrow = bool(env_config.get("add_target_velocity_arrow", False))
        ...
        self.spawn_target_in_heading_cone = bool(self.stage_config["spawn_in_heading_cone"])
        self.target_spawn_half_angle = np.deg2rad(float(self.stage_config["spawn_half_angle_degrees"]))
        self.target_speed_min = float(env_config["target"]["speed_min"])
        self.target_speed_max = float(env_config["target"]["speed_max"])
        ...
        if self.should_render and self.add_target_velocity_arrow:
        ...
        self.point_spawn_radius = float(self.stage_config["target_spawn_radius"])
        self.point_spawn_radius_min = float(self.stage_config.get("target_spawn_radius_min", self.point_spawn_radius))
        self.curriculum_point_spawn_radius = bool(self.stage_config.get("curriculum_target_spawn_radius", False))
    ...
    def sample_target_velocity(self, key):
        direction_key, speed_key = jax.random.split(key)
        target_direction = jax.random.uniform(direction_key, minval=-jnp.pi, maxval=jnp.pi)
        target_speed = jax.random.uniform(speed_key, minval=self.target_speed_min, maxval=self.target_speed_max)
        return target_speed * jnp.array([jnp.cos(target_direction), jnp.sin(target_direction)])


    def sample_point_reset(self, qpos, qvel, internal_state, key):
        spawn_key, target_velocity_key = jax.random.split(key)
        ...
        if self.spawn_target_in_heading_cone:
            relative_angle = jax.random.uniform(spawn_key, minval=-self.target_spawn_half_angle, maxval=self.target_spawn_half_angle)
            angle = self.root_yaw_from_qpos(qpos) + relative_angle
        else:
            angle = jax.random.uniform(spawn_key, minval=-jnp.pi, maxval=jnp.pi)
        ...
        target_velocity = self.sample_target_velocity(target_velocity_key)

        qpos = qpos.at[self.point_qposadr:self.point_qposadr + 7].set(point_qpos)
        qvel = qvel.at[self.point_qveladr:self.point_qveladr + 6].set(jnp.concatenate([target_velocity, jnp.zeros(4)]))
        internal_state["target_velocity_world"] = target_velocity

        return qpos, qvel


    def advance_target(self, point_qpos, internal_state):
        target_xy = point_qpos[:2] + internal_state["target_velocity_world"] * self.dt
        target_z = self.terrain_function.ground_height_at(internal_state, target_xy[0], target_xy[1]) + self.point_radius
        return point_qpos.at[:3].set(jnp.array([target_xy[0], target_xy[1], target_z]))
    ...
    def update_point_sensing(self, data, internal_state, info, reset_timer):
        target_xy_distance_to_base = jnp.linalg.norm(
            self.point_position_world(data)[:2] - self.base_position_world(data)[:2]
        )
        info["env_info/target_xy_distance_to_base"] = target_xy_distance_to_base
        info["env_info/target_xy_distance_to_com"] = target_xy_distance_to_base
        info["env_info/point_xy_distance_to_base"] = target_xy_distance_to_base
        info["env_info/point_xy_distance_to_com"] = target_xy_distance_to_base
    ...
    def render(self, state):
        ...
        if self.add_target_velocity_arrow:
            target_velocity = state.internal_state["target_velocity_world"][env_id]
            trunk_rotation = state.internal_state["imu_orientation_euler"][env_id][2]
            desired_angle = trunk_rotation + np.arctan2(target_velocity[1], target_velocity[0])
            ...
            magnitude = np.linalg.norm(target_velocity)
    ...
    def reset(self, key, eval_mode):
        ...
        internal_state = {
            ...
            "target_velocity_world": jnp.array([0.0, 0.0]),
    ...
    def _reset(self, state):
        key, initial_state_key, terrain_key, domain_randomization_key, observation_key, gait_manager_key, point_reset_key = jax.random.split(state.key, 7)
    ...
    def _step(self, state, action):
        key, action_delay_key, domain_randomization_key, observation_key, terrain_key = jax.random.split(state.key, 5)
        state = state.replace(key=key)
        ...
        target_qpos = self.advance_target(state.data.qpos[self.point_qposadr:self.point_qposadr + 7], state.internal_state)
        target_qvel = jnp.concatenate([state.internal_state["target_velocity_world"], jnp.zeros(4)])
        data = data.replace(
            qpos=data.qpos.at[self.point_qposadr:self.point_qposadr + 7].set(target_qpos),
            qvel=data.qvel.at[self.point_qveladr:self.point_qveladr + 6].set(target_qvel),
        )
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        ...
        point_pos_world = self.point_position_world(data)
        point_vel_world = self.point_velocity_world(data)
        ...
        relative_point_position = self.relative_point_position_base(data, internal_state, base_pos_world, base_yaw)
        ...
        observation = observation.at[self.relative_point_position_obs_idx].set(jnp.clip(observation[self.relative_point_position_obs_idx] / self.point_spawn_radius, -1.0, 1.0))
        ...
        observation = observation.at[self.point_position_world_obs_idx].set(jnp.clip(observation[self.point_position_world_obs_idx] / self.point_spawn_radius, -1.0, 1.0))
        observation = observation.at[self.point_velocity_world_obs_idx].set(jnp.clip(observation[self.point_velocity_world_obs_idx] / self.target_speed_max, -1.0, 1.0))
        observation = observation.at[self.base_position_world_obs_idx].set(jnp.clip(observation[self.base_position_world_obs_idx] / self.point_spawn_radius, -1.0, 1.0))
    ...
    def get_observation_space(self):
        ...
        self.relative_point_position_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
        ...
        self.point_position_world_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
        self.point_velocity_world_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
