...
class DefaultReward:
    ...
    def step(self, data, internal_state):
        ...
        internal_state["feet_time_on_ground"] = jnp.where(
            feet_floor_contacts,
            internal_state["feet_time_on_ground"] + self.env.dt,
            0.0,
        )
        internal_state["feet_time_in_air"] = jnp.where(
            feet_floor_contacts,
            0.0,
            internal_state["feet_time_in_air"] + self.env.dt,
        )
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        target_pos_world = self.env.point_position_world(data)
        target_vel_world = self.env.point_velocity_world(data)
        base_pos_world = self.env.base_position_world(data)
        base_yaw = internal_state["imu_orientation_euler"][2]

        current_imu_angular_velocity = data.sensordata[
        ...
        follow_distance = jnp.linalg.norm(base_pos_world[:2] - target_pos_world[:2])
        follow_reward_raw = jnp.exp(-0.5 * follow_distance)
        ...
        info["reward/follow"] = follow_reward
        info["env_info/follow_reward_raw"] = follow_reward_raw
        info["env_info/follow_distance"] = follow_distance
        info["reward/total"] = reward
        info["env_info/target_xy_distance_to_base"] = follow_distance
        info["env_info/target_distance_to_base"] = jnp.linalg.norm(target_pos_world - base_pos_world)
        info["env_info/target_speed"] = jnp.linalg.norm(target_vel_world[:2])
        info["env_info/target_velocity_x"] = target_vel_world[0]
        info["env_info/target_velocity_y"] = target_vel_world[1]
