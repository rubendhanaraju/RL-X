...
class DefaultReward:
    ...
    def setup(self):
        ...
        self.env.internal_state["point_reached"] = False
    ...
    def reward_yaw_alignment(self, base_yaw, base_xy, point_xy, point_velocity_command_xy):
        cmd_speed = np.linalg.norm(point_velocity_command_xy)
        cmd_yaw = np.where(
            cmd_speed > 1e-6,
            np.arctan2(point_velocity_command_xy[1], point_velocity_command_xy[0]),
            base_yaw,
        )
        point_rel = point_xy - base_xy
        point_yaw = np.arctan2(point_rel[1], point_rel[0])
        err_cmd = self.wrap_to_pi(cmd_yaw - base_yaw)
        err_point = self.wrap_to_pi(point_yaw - base_yaw)
        return np.exp(-2.0 * (err_cmd**2 + err_point**2))
    ...
    def reward_and_info(self, action):
        ...
        point_yaw = np.arctan2(point_pos_world[1] - base_pos_world[1], point_pos_world[0] - base_pos_world[0])
        command_speed = np.linalg.norm(point_velocity_command_xy)
        command_yaw = np.where(
            command_speed > 1e-6,
            np.arctan2(point_velocity_command_xy[1], point_velocity_command_xy[0]),
            base_yaw,
        )
        ...
        self.env.internal_state["info"]["reward/progress_to_point"] = progress_to_point_reward
        self.env.internal_state["info"]["env_info/point_reached"] = np.float32(point_reached)
        self.env.internal_state["info"]["env_info/point_reached_now"] = np.float32(point_reached_now)
        ...
        self.env.internal_state["info"]["env_info/point_yaw"] = point_yaw
        self.env.internal_state["info"]["env_info/point_command_yaw"] = command_yaw
        self.env.internal_state["info"]["env_info/point_yaw_error"] = point_yaw_error
        ...
        self.env.internal_state["info"]["env_info/point_velocity_command_x"] = point_velocity_command_xy[0]
        self.env.internal_state["info"]["env_info/point_velocity_command_y"] = point_velocity_command_xy[1]
