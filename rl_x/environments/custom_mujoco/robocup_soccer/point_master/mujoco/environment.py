...
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mujoco.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
...
class PointMasterEnv(gym.Env):
    def __init__(self, robot_config, runner_mode, seed, render, env_config, nr_envs):
        ...
        self.internal_state = {
            ...
            "point_velocity_command": np.array([0.0, 0.0]),
    ...
    def render(self):
        ...
        if self.runner_mode == "test":
            ...
            if explicit_velocity_commands:
                point_velocity_command = np.array([goal_x_velocity, goal_y_velocity])
                point_velocity_command = np.where(np.abs(point_velocity_command) < (self.command_function.zero_clip_threshold_percentage * self.internal_state["max_point_velocity"]), 0.0, point_velocity_command)
                self.internal_state["point_velocity_command"] = np.clip(point_velocity_command, -self.internal_state["max_point_velocity"], self.internal_state["max_point_velocity"])
                actuator_keep_nominal_commands = np.where(np.all(point_velocity_command == 0.0), np.ones(self.nr_actuator_joints, dtype=bool), self.command_function.default_actuator_joint_keep_nominal)
                self.internal_state["actuator_joint_keep_nominal"] = actuator_keep_nominal_commands

        if self.add_goal_arrow:
            point_velocity_command = self.internal_state["point_velocity_command"]
    ...
    def get_observation(self, action):
        ...
        observation[self.point_velocity_command_obs_idx] = np.clip(observation[self.point_velocity_command_obs_idx] / self.internal_state["max_point_velocity"], -1.0, 1.0)
    ...
    def get_observation_space(self):
        ...
        self.point_velocity_command_obs_idx = np.array([current_observation_idx + i for i in range(2)], dtype=int)
