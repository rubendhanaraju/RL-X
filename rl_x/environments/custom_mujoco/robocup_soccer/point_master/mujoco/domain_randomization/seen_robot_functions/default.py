...
class DefaultDRSeenRobotFunction:
    def __init__(self, env):
        ...
        self.torque_limit_min_factor = env.env_config["domain_randomization"]["seen_robot"].get("torque_limit_min_factor", -self.torque_limit_factor)
        self.torque_limit_max_factor = env.env_config["domain_randomization"]["seen_robot"].get("torque_limit_max_factor", self.torque_limit_factor)
        ...
        self.add_base_mass = env.env_config["domain_randomization"]["seen_robot"].get("add_base_mass", 0.0)
    ...
    def sample(self):
        ...
        seen_body_masses[base_mass_idx] = np.maximum(
            0.01,
            seen_body_masses[base_mass_idx] + env_curriculum_coeff * self.env.np_rng.uniform(low=-self.add_base_mass, high=self.add_base_mass),
        )
        ...
        seen_coms = default_coms.copy()
        base_com_idx = self.env.trunk_body_id - 1
        seen_coms[base_com_idx] += env_curriculum_coeff * self.env.np_rng.uniform(low=-self.add_com_displacement, high=self.add_com_displacement, size=(3,))
        ...
        torque_limits = default_torque_limits * (1 + env_curriculum_coeff * self.env.np_rng.uniform(low=self.torque_limit_min_factor, high=self.torque_limit_max_factor, size=self.default_torque_limits.shape))
