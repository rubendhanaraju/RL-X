class DefaultDRObservationNoise:
    ...
    def modify_observation(self, observation):
        ...
        observation[self.relative_point_position_obs_idx] += self.env.internal_state["env_curriculum_coeff"] * self.env.np_rng.uniform(size=(len(self.relative_point_position_obs_idx),), low=-self.point_position_noise, high=self.point_position_noise)
