...
class DefaultGaitManager:
    def __init__(self, env):
        ...
        self.randomize_phase = env.env_config["gait_manager"].get("randomize_phase", False)
    ...
    def setup(self):
        ...
        default_phase_offsets = np.array([0.0, -np.pi])
        gait_phase_offset = np.where(self.randomize_phase and not self.env.internal_state["in_eval_mode"], random_phase_offsets, default_phase_offsets)
    ...
    def get_phase_for_reward(self):
        phase_tp1 = self.wrap_to_pi(self.env.internal_state["gait_phase"] + self.env.internal_state["gait_phase_dt"])
        is_standing_command = np.all(self.env.internal_state["point_velocity_command"] == 0.0)
