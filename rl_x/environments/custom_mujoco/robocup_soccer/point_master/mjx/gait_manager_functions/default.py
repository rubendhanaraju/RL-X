...
class DefaultGaitManager:
    ...
    def setup(self, internal_state, key):
        ...
        default_phase_offsets = jnp.array([0.0, -jnp.pi])
        if self.randomize_phase:
            gait_phase_offset = jnp.where(internal_state["in_eval_mode"], default_phase_offsets, random_phase_offsets)
        else:
            gait_phase_offset = default_phase_offsets
    ...
    def get_phase_for_reward(self, internal_state):
        phase_tp1 = self.wrap_to_pi(internal_state["gait_phase"] + internal_state["gait_phase_dt"])
        is_standing_command = jnp.all(internal_state["point_velocity_command"] == 0.0)
