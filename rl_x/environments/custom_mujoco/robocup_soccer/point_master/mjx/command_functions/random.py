...
class RandomCommands:
    ...
    def get_next_command(self, internal_state, should_sample_commands, subkey):
        ...
        point_velocity_command = jax.random.uniform(velocity_sampling_key, (2,), minval=-internal_state["max_point_velocity"], maxval=internal_state["max_point_velocity"])
        point_velocity_command = jnp.where(jnp.abs(point_velocity_command) < (self.zero_clip_threshold_percentage * internal_state["max_point_velocity"]), 0.0, point_velocity_command)
        point_velocity_command = jnp.where(jax.random.bernoulli(all_zeroing_key, self.all_zero_chance), jnp.zeros(2), point_velocity_command)
        point_velocity_command = jnp.where(jax.random.uniform(single_zeroing_key, (2,)) < self.single_zero_chance, 0.0, point_velocity_command)

        internal_state["point_velocity_command"] = jnp.where(should_sample_commands, point_velocity_command, internal_state["point_velocity_command"])

        actuator_joint_keep_nominal = jnp.where(jnp.all(point_velocity_command == 0.0), jnp.ones(self.env.nr_actuator_joints, dtype=bool), self.default_actuator_joint_keep_nominal)
