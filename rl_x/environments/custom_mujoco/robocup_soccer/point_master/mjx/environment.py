...
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.state import State
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.point_master.mjx.terrain_functions.handler import get_terrain_function
...
class PointMasterEnv:
    ...
    def render(self, state):
        ...
        if self.runner_mode == "test":
            ...
            if explicit_velocity_commands:
                point_velocity_command = jnp.array([goal_x_velocity, goal_y_velocity])
                point_velocity_command = jnp.where(jnp.abs(point_velocity_command) < (self.command_function.zero_clip_threshold_percentage * state.internal_state["max_point_velocity"]), 0.0, point_velocity_command)
                point_velocity_command = jnp.clip(point_velocity_command, -state.internal_state["max_point_velocity"], state.internal_state["max_point_velocity"])
                state.internal_state["point_velocity_command"] = jnp.tile(point_velocity_command, (self.nr_envs, 1))
                actuator_joint_keep_nominal = jnp.where(jnp.all(point_velocity_command == 0.0), jnp.ones(self.nr_actuator_joints, dtype=bool), self.command_function.default_actuator_joint_keep_nominal)
        ...
        if self.add_goal_arrow:
            point_velocity_command = state.internal_state["point_velocity_command"][env_id]
            trunk_rotation = state.internal_state["imu_orientation_euler"][env_id][2]
            desired_angle = trunk_rotation + np.arctan2(point_velocity_command[1], point_velocity_command[0])
            ...
            magnitude = np.sqrt(np.sum(np.square([point_velocity_command[0], point_velocity_command[1]])))
    ...
    def reset(self, key, eval_mode):
        ...
        internal_state = {
            ...
            "point_velocity_command": jnp.array([0.0, 0.0]),
    ...
    def _step(self, state, action):
        ...
        if self.command_resampling_steps > 0:
            should_sample_commands = ((state.info_episode_store["episode_step"] + 1) % self.command_resampling_steps) == 0
        else:
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        ...
        base_pos_world = self.get_base_position_fcp(data)
        ...
        critic_base_pos_world = self.get_base_position_true(data)
        ...
        critic_relative_point_position = self.relative_point_position_base(data, internal_state, critic_base_pos_world, critic_base_yaw)
        ...
        observation = jnp.concatenate([
            ...
            body_orientation,
            internal_state["point_velocity_command"],
        ...
        observation = observation.at[self.point_velocity_command_obs_idx].set(jnp.clip(observation[self.point_velocity_command_obs_idx] / internal_state["max_point_velocity"], -1.0, 1.0))
        ...
        observation = observation.at[self.point_velocity_world_obs_idx].set(jnp.clip(observation[self.point_velocity_world_obs_idx] / internal_state["max_point_velocity"], -1.0, 1.0))
        ...
        observation = observation.at[self.critic_relative_point_position_true_obs_idx].set(jnp.clip(observation[self.critic_relative_point_position_true_obs_idx] / self.point_spawn_radius, -1.0, 1.0))
        observation = observation.at[self.critic_base_position_world_true_obs_idx].set(jnp.clip(observation[self.critic_base_position_world_true_obs_idx] / self.point_spawn_radius, -1.0, 1.0))
    ...
    def get_observation_space(self):
        ...
        self.point_velocity_command_obs_idx = jnp.array([current_observation_idx + i for i in range(2)])
        ...
        current_observation_idx += 3
        self.critic_relative_point_position_true_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
        self.critic_gravity_vector_true_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
        self.critic_base_position_world_true_obs_idx = jnp.array([current_observation_idx + i for i in range(3)])
        current_observation_idx += 3
