...
class PointMasterEnv:
    ...
    def relative_point_position_base(self, data, internal_state):
        point_pos = self.point_position_world(data)
        base_pos = self.base_position_world(data)
        point_rel_base_xy = self.rotate_world_to_base_xy(point_pos[:2] - base_pos[:2], internal_state["imu_orientation_euler"][2])
    ...
    def update_point_sensing(self, data, internal_state, info, reset_timer):
        info["env_info/point_xy_distance_to_com"] = jnp.linalg.norm(
            self.point_position_world(data)[:2] - self.robot_com_position_world(data)[:2]
        )
    ...
    def _reset(self, state):
        ...
        new_state.internal_state["previous_point_distance_to_com"] = jnp.linalg.norm(self.point_position_world(data)[:2] - self.robot_com_position_world(data)[:2])
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        ...
        relative_point_position = self.relative_point_position_base(data, internal_state)
