...
def convert_policy(flax_policy_params, policy_observation_indices):
    ...
    expected_policy_obs_dim = int(np.asarray(p["Dense_0"]["kernel"]).shape[0])
    action_dim = int(np.asarray(p["Dense_3"]["kernel"]).shape[1])
    ...
    state_dict = {}
    copy_linear(state_dict, "dense1", p["Dense_0"])
    copy_layernorm(state_dict, "ln1", p["LayerNorm_0"])
    copy_linear(state_dict, "dense2", p["Dense_1"])
    copy_linear(state_dict, "dense3", p["Dense_2"])
    copy_linear(state_dict, "mean_head", p["Dense_3"])
...
def build_initial_policy_state(config, env):
    ...
    learning_rate = (
        linear_schedule if config.algorithm.anneal_learning_rate else config.algorithm.learning_rate
    )
...
def main():
    ...
    parser.add_argument("--output", type=str, default="policy_ff.pth")
    parser.add_argument("--meta-output", type=str, default="policy_ff_meta.json")
    ...
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        torch_model = TorchPolicyFeedforward.from_meta(meta)
