...
class PDControl:
    ...
    def process_action(self, action, internal_state):
        scaled_action = action * self.env.action_control_mask * internal_state["scaling_factor"]
        target_joint_positions = internal_state["actuator_joint_nominal_positions"] + scaled_action
        noisy_target_joint_positions = target_joint_positions + (internal_state["position_offsets"] * self.env.action_control_mask)
