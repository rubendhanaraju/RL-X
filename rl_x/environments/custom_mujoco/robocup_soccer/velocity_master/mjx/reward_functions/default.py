...
class DefaultReward:
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        yaw_velocity_difference_norm = jnp.square(yaw_velocity_difference)
        ...
        robot_imu_height = internal_state["robot_imu_height_over_ground"]
        robot_imu_height_ratio = robot_imu_height / jnp.maximum(internal_state["robot_nominal_imu_height_over_ground"], 1e-6)
        height_threshold = self.height_ratio_threshold * internal_state["robot_nominal_imu_height_over_ground"]
        ...
        info["env_info/height_ratio_threshold"] = self.height_ratio_threshold
        info["env_info/height_threshold"] = height_threshold
        ...
        info["env_info/xy_vel_diff_abs"] = jnp.nan_to_num(
            jnp.mean(jnp.minimum(jnp.abs(xy_difference), 2 * internal_state["max_command_velocity"])),
            nan=2 * internal_state["max_command_velocity"],
            posinf=2 * internal_state["max_command_velocity"],
            neginf=2 * internal_state["max_command_velocity"],
        )
        info["env_info/yaw_vel_diff_abs"] = jnp.abs(yaw_velocity_difference)
        info["env_info/command_velocity_x"] = desired_imu_linear_velocity_xy[0]
        info["env_info/command_velocity_y"] = desired_imu_linear_velocity_xy[1]
        info["env_info/command_yaw_velocity"] = desired_imu_yaw_velocity
        info["env_info/current_velocity_x"] = current_imu_linear_velocity[0]
        info["env_info/current_velocity_y"] = current_imu_linear_velocity[1]
