...
class RandomCommands:
    ...
    def get_next_command(self, internal_state, should_sample_commands, subkey):
        ...
        full_goal_velocities = jax.random.uniform(
            velocity_sampling_key,
            (3,),
            minval=-internal_state["max_command_velocity"],
            maxval=internal_state["max_command_velocity"],
        )
        frontal_goal_velocities = jnp.array([
            jax.random.uniform(
                velocity_sampling_key,
                minval=0.0,
                maxval=internal_state["max_command_velocity"],
        ...
        goal_velocities = jnp.where(
            internal_state["command_full_velocity_unlocked"],
            full_goal_velocities,
            frontal_goal_velocities,
        )
