...
class DefaultDRPerturbation:
    ...
    def sample(self, internal_state, mjx_model, data, should_randomize, key):
        ...
        joint_velocity = data.qvel[self.env.actuator_joint_mask_qvel] + internal_state["env_curriculum_coeff"] * jax.random.uniform(joint_velocity_key, (self.env.nr_actuator_joints,), minval=-self.max_joint_velocity, maxval=self.max_joint_velocity)
        joint_velocity = jnp.where(should_randomize, joint_velocity, data.qvel[self.env.actuator_joint_mask_qvel])
        joint_position = data.qpos[self.env.actuator_joint_mask_qpos] + internal_state["env_curriculum_coeff"] * jax.random.uniform(joint_position_key, (self.env.nr_actuator_joints,), minval=-self.max_joint_position, maxval=self.max_joint_position)
        joint_position = jnp.where(should_randomize, joint_position, data.qpos[self.env.actuator_joint_mask_qpos])

        data = data.replace(
            qpos=data.qpos.at[self.env.actuator_joint_mask_qpos].set(joint_position),
            qvel=data.qvel.at[:6].set(trunk_velocity).at[self.env.actuator_joint_mask_qvel].set(joint_velocity)
        )
