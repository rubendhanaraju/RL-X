...
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.velocity_master.mujoco.terrain_functions.handler import get_terrain_function
...
class VelocityMasterEnv(gym.Env):
    def __init__(self, robot_config, runner_mode, seed, render, env_config, nr_envs):
        ...
        self.internal_state = {
            ...
            "max_command_velocity": self.command_velocity_curriculum_initial,
            "max_command_velocity_limit": self.max_command_velocity_limit,
            "command_velocity_curriculum_successes": 0,
            "command_velocity_curriculum_level": 0,
            "command_full_velocity_unlocked": False,
            "command_frontal_curriculum_successes_in_a_row": 0,
            "command_resampling_probability": min(self.command_resampling_probability_curriculum_initial, self.command_resampling_probability_limit),
            "command_resampling_probability_limit": self.command_resampling_probability_limit,
            "command_resampling_probability_curriculum_successes": 0,
            "command_resampling_probability_curriculum_level": 0,
    ...
    def sample_point_reset(self, qpos, qvel):
        if self.curriculum_point_spawn_radius:
            point_spawn_radius = self.point_spawn_radius_min + self.internal_state["env_curriculum_coeff"] * (self.point_spawn_radius - self.point_spawn_radius_min)
        else:
            point_spawn_radius = self.point_spawn_radius

        if self.spawn_point_in_heading_cone:
            relative_angle = self.np_rng.uniform(low=-self.point_spawn_half_angle, high=self.point_spawn_half_angle)
            angle = self.root_yaw_from_qpos(qpos) + relative_angle
        else:
            angle = self.np_rng.uniform(low=-np.pi, high=np.pi)
        point_xy = qpos[:2] + point_spawn_radius * np.array([np.cos(angle), np.sin(angle)])
        point_z = self.terrain_function.ground_height_at(point_xy[0], point_xy[1]) + self.point_radius
        point_qpos = np.array([point_xy[0], point_xy[1], point_z, 1.0, 0.0, 0.0, 0.0])

        qpos[self.point_qposadr:self.point_qposadr + 7] = point_qpos
        qvel[self.point_qveladr:self.point_qveladr + 6] = np.zeros(6)

        return qpos, qvel


    def point_position_world(self):
        return self.internal_state["data"].qpos[self.point_qposadr:self.point_qposadr + 3]


    def point_velocity_world(self):
        return self.internal_state["data"].qvel[self.point_qveladr:self.point_qveladr + 3]
    ...
    def command_tracking_curriculum_metrics(self):
        reached_horizon = self.internal_state["info_episode_store"]["episode_step"] >= self.horizon
        episode_steps = max(self.internal_state["info_episode_store"]["episode_step"], 1)
        commanded_steps = self.internal_state["info_episode_store"]["episode_commanded_steps"]
        commanded_fraction = commanded_steps / episode_steps
        avg_xy_tracking_error = self.internal_state["info_episode_store"]["episode_total_xy_velocity_diff_abs"] / max(commanded_steps, 1)
        avg_yaw_tracking_error = self.internal_state["info_episode_store"]["episode_total_yaw_velocity_diff_abs"] / max(commanded_steps, 1)
        tracked_sampled_velocity = (
            commanded_fraction >= self.command_velocity_curriculum_min_commanded_fraction
            and avg_xy_tracking_error <= self.command_velocity_curriculum_xy_error_threshold
            and avg_yaw_tracking_error <= self.command_velocity_curriculum_yaw_error_threshold
        )
        return reached_horizon, tracked_sampled_velocity, commanded_fraction, avg_xy_tracking_error, avg_yaw_tracking_error


    def update_command_velocity_curriculum(self):
        if self.internal_state["in_eval_mode"]:
            self.internal_state["max_command_velocity"] = self.internal_state["max_command_velocity_limit"]
            self.internal_state["command_full_velocity_unlocked"] = True
            self.internal_state["command_resampling_probability"] = self.internal_state["command_resampling_probability_limit"]
            return

        reached_horizon, tracked_sampled_velocity, _, _, _ = self.command_tracking_curriculum_metrics()
        was_full_velocity_unlocked = self.internal_state["command_full_velocity_unlocked"]
        reached_frontal_curriculum_step_goal = (
            self.internal_state["info_episode_store"]["episode_step"] >= self.command_frontal_curriculum_min_episode_steps
        )
        if not was_full_velocity_unlocked:
            if reached_frontal_curriculum_step_goal:
                self.internal_state["command_frontal_curriculum_successes_in_a_row"] += 1
            else:
                self.internal_state["command_frontal_curriculum_successes_in_a_row"] = 0
            if self.internal_state["command_frontal_curriculum_successes_in_a_row"] >= self.command_frontal_curriculum_successes_in_a_row:
                self.internal_state["command_full_velocity_unlocked"] = True

        below_limit = self.internal_state["max_command_velocity"] < (self.internal_state["max_command_velocity_limit"] - 1e-6)
        if reached_horizon and tracked_sampled_velocity and below_limit:
            self.internal_state["command_velocity_curriculum_successes"] += 1

        should_ramp = (
            self.internal_state["command_velocity_curriculum_successes"] >= self.command_velocity_curriculum_successes_per_level
            and below_limit
        )
        if should_ramp:
            self.internal_state["max_command_velocity"] = min(
                self.internal_state["max_command_velocity"] + self.command_velocity_curriculum_increment,
                self.internal_state["max_command_velocity_limit"],
            )
            self.internal_state["command_velocity_curriculum_level"] += 1
            self.internal_state["command_velocity_curriculum_successes"] = 0

        uses_probability_resampling = self.command_resampling_steps == 0
        resampling_below_limit = self.internal_state["command_resampling_probability"] < (self.internal_state["command_resampling_probability_limit"] - 1e-9)
        if uses_probability_resampling and was_full_velocity_unlocked and reached_horizon and tracked_sampled_velocity and resampling_below_limit:
            self.internal_state["command_resampling_probability_curriculum_successes"] += 1

        should_ramp_resampling = (
            uses_probability_resampling
            and was_full_velocity_unlocked
            and self.internal_state["command_resampling_probability_curriculum_successes"] >= self.command_resampling_probability_curriculum_successes_per_level
            and resampling_below_limit
        )
        if should_ramp_resampling:
            self.internal_state["command_resampling_probability"] = min(
                self.internal_state["command_resampling_probability"] + self.command_resampling_probability_curriculum_increment,
                self.internal_state["command_resampling_probability_limit"],
            )
            self.internal_state["command_resampling_probability_curriculum_level"] += 1
            self.internal_state["command_resampling_probability_curriculum_successes"] = 0


    def update_command_velocity_curriculum_info(self):
        self.internal_state["info"]["command_curriculum/max_command_velocity"] = self.internal_state["max_command_velocity"]
        self.internal_state["info"]["command_curriculum/max_command_velocity_limit"] = self.internal_state["max_command_velocity_limit"]
        self.internal_state["info"]["command_curriculum/successes"] = self.internal_state["command_velocity_curriculum_successes"]
        self.internal_state["info"]["command_curriculum/level"] = self.internal_state["command_velocity_curriculum_level"]
        self.internal_state["info"]["command_curriculum/full_velocity_unlocked"] = float(self.internal_state["command_full_velocity_unlocked"])
        self.internal_state["info"]["command_curriculum/frontal_successes_in_a_row"] = self.internal_state["command_frontal_curriculum_successes_in_a_row"]
        self.internal_state["info"]["command_curriculum/resampling_probability"] = self.internal_state["command_resampling_probability"]
        self.internal_state["info"]["command_curriculum/resampling_probability_limit"] = self.internal_state["command_resampling_probability_limit"]
        self.internal_state["info"]["command_curriculum/resampling_successes"] = self.internal_state["command_resampling_probability_curriculum_successes"]
        self.internal_state["info"]["command_curriculum/resampling_level"] = self.internal_state["command_resampling_probability_curriculum_level"]
        _, _, commanded_fraction, avg_xy_tracking_error, avg_yaw_tracking_error = self.command_tracking_curriculum_metrics()
        self.internal_state["info"]["command_curriculum/commanded_fraction"] = commanded_fraction
        self.internal_state["info"]["command_curriculum/avg_xy_tracking_error"] = avg_xy_tracking_error
        self.internal_state["info"]["command_curriculum/avg_yaw_tracking_error"] = avg_yaw_tracking_error
    ...
    def render(self):
        ...
        if self.add_goal_arrow:
            ...
            self.internal_state["data"].site("dir_arrow_point").xpos = self.internal_state["data"].body("dir_arrow").xpos + [-0.1 * np.sin(np.pi/2 + desired_angle), 0.1 * np.cos(np.pi/2 + desired_angle), 0]
    ...
    def reset(self, seed=None):
        ...
        self.update_command_velocity_curriculum()

        self.terrain_function.sample()

        qpos, qvel = self.initial_state_function.setup()
        qpos, qvel = self.sample_point_reset(qpos, qvel)
        ...
        self.update_command_velocity_curriculum_info()
    ...
    def step(self, action):
        ...
        self.internal_state["data"].qvel[self.point_qveladr:self.point_qveladr + 6] = np.zeros(6)
        mujoco.mj_forward(self.internal_state["mj_model"], self.internal_state["data"])
        ...
        self.update_point_sensing(reset_timer=False)

        reward = self.reward_function.reward_and_info(chosen_action)

        command_active = np.any(np.abs(self.internal_state["goal_velocities"]) > 1e-6)
        if command_active:
            ...
            self.internal_state["info_episode_store"]["episode_total_yaw_velocity_diff_abs"] += self.internal_state["info"]["env_info/yaw_vel_diff_abs"]
            self.internal_state["info_episode_store"]["episode_commanded_steps"] += 1

        allow_mid_episode_command_resampling = self.internal_state["command_full_velocity_unlocked"]
        if self.command_resampling_steps > 0:
            should_sample_commands = (
                allow_mid_episode_command_resampling
                and ((self.internal_state["info_episode_store"]["episode_step"] + 1) % self.command_resampling_steps) == 0
            )
        else:
            should_sample_commands = allow_mid_episode_command_resampling and self.command_sampling_function.step()
    ...
    def get_observation(self, action):
        ...
        observation[self.goal_velocities_obs_idx] = np.clip(observation[self.goal_velocities_obs_idx] / self.internal_state["max_command_velocity"], -1.0, 1.0)
    ...
    def get_observation_space(self):
        ...
        current_observation_idx += 3
        self.relative_point_position_obs_idx = np.array([], dtype=int)
        ...
        self.point_position_world_obs_idx = np.array([], dtype=int)
        self.point_velocity_world_obs_idx = np.array([], dtype=int)
        self.base_position_world_obs_idx = np.array([], dtype=int)
