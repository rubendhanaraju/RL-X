...
class RandomCommands:
    ...
    def get_next_command(self):
        if self.env.internal_state.get("command_full_velocity_unlocked", False):
            goal_velocities = self.env.np_rng.uniform(
                size=(3,),
                low=-self.env.internal_state["max_command_velocity"],
                high=self.env.internal_state["max_command_velocity"],
            )
        else:
            goal_velocities = np.array([
                self.env.np_rng.uniform(low=0.0, high=self.env.internal_state["max_command_velocity"]),
                0.0,
                0.0,
            ])
