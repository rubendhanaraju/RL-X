class DefaultDRObservationNoise:
    ...
    def modify_observation(self, observation):
        ...
        observation[self.relative_ball_position_obs_idx] += self.env.internal_state["env_curriculum_coeff"] * self.env.np_rng.uniform(size=(len(self.relative_ball_position_obs_idx),), low=-self.ball_position_noise, high=self.ball_position_noise)
