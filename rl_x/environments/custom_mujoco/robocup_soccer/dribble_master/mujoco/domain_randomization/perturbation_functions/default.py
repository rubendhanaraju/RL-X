...
class DefaultDRPerturbation:
    ...
    def sample(self):
        ...
        joint_velocity = self.env.internal_state["data"].qvel[self.env.actuator_joint_mask_qvel] + self.env.internal_state["env_curriculum_coeff"] * self.env.np_rng.uniform(size=(self.env.nr_actuator_joints,), low=-self.max_joint_velocity, high=self.max_joint_velocity)
        joint_position = self.env.internal_state["data"].qpos[self.env.actuator_joint_mask_qpos] + self.env.internal_state["env_curriculum_coeff"] * self.env.np_rng.uniform(size=(self.env.nr_actuator_joints,), low=-self.max_joint_position, high=self.max_joint_position)

        self.env.internal_state["data"].qpos[self.env.actuator_joint_mask_qpos] = joint_position
        self.env.internal_state["data"].qvel[:6] = trunk_velocity
        self.env.internal_state["data"].qvel[self.env.actuator_joint_mask_qvel] = joint_velocity
