...
class DefaultDRSeenRobotFunction:
    ...
    def sample(self, internal_state, mjx_model, data, should_randomize, key):
        ...
        base_mass = jnp.maximum(
            0.01,
            seen_body_masses[base_mass_idx] + env_curriculum_coeff * jax.random.uniform(keys[29], minval=-self.add_base_mass, maxval=self.add_base_mass),
        )
        seen_body_masses = seen_body_masses.at[base_mass_idx].set(base_mass)
        ...
        base_com_offset = env_curriculum_coeff * jax.random.uniform(keys[4], minval=-self.add_com_displacement, maxval=self.add_com_displacement, shape=(3,))
        seen_coms = default_coms.at[base_com_idx].set(default_coms[base_com_idx] + base_com_offset)
        ...
        torque_limits = default_torque_limits * (1 + env_curriculum_coeff * jax.random.uniform(keys[14], minval=self.torque_limit_min_factor, maxval=self.torque_limit_max_factor, shape=self.default_torque_limits.shape))
