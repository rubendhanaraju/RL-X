...
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.locomotion_ball.mujoco.terrain_functions.handler import get_terrain_function


class LocomotionBallEnv(gym.Env):
    def __init__(self, robot_config, runner_mode, seed, render, env_config, nr_envs):
        ...
        self.head_actuator_joint_mask = head_actuator_joint_mask.astype(np.float32)
        self.actuator_joint_nominal_diff_weights = actuator_joint_nominal_diff_weights
        ...
        self.foot_lateral_half_widths = self.initial_mj_model.geom_size[self.foot_geom_indices, 1]
        self.target_feet_inner_gap = 2.0 * self.ball_radius
        ...
        self.command_resampling_episode_length_thresholds = np.asarray(
        ...
        self.ball_activation_episode_length_threshold = float(
            task_curriculum_config.get("ball_activation_episode_length_threshold", 1000)
        )
        ...
        self.inactive_ball_rel_xy = np.asarray(
            [
        ...
        self.internal_state = {
            ...
            "env_curriculum_coeff": 1.0 if eval_mode else 0.0,
            "env_curriculum_levels_in_a_row": 0.0,
            "command_resampling_level": 0.0,
            "command_resampling_streak": 0.0,
            "command_resampling_curriculum_coeff": 0.0,
            "locomotion_task_stage": self.initial_task_stage,
            "locomotion_task_stage_success_streak": 0.0,
            ...
            "info": {
                ...
                "env_curriculum/coefficient": 1.0 if eval_mode else 0.0,
            },
            "info_episode_store": {
                "episode_return": 0.0,
                "episode_step": 0,
    ...
    def sample_ball_reset(self, qpos, qvel):
        ...
        if not self.is_ball_task_active():
            ball_rel_base = self.inactive_ball_rel_xy
    ...
    def ball_velocity_base(self):
        ball_vel = self.ball_velocity_world()
        ball_vel_base_xy = self.rotate_world_to_base_xy(ball_vel[:2], self.internal_state["imu_orientation_euler"][2])
        return np.concatenate([ball_vel_base_xy, ball_vel[2:3]])


    def trunc2(self, value):
        return np.trunc(value * 100.0) / 100.0


    def sense_ball(self):
        camera_pos = self.internal_state["data"].site_xpos[self.camera_site_id]
        ball_pos = self.internal_state["data"].site_xpos[self.ball_site_id]
        base_yaw = self.root_yaw_from_qpos(self.internal_state["data"].qpos)
        local_xy = self.rotate_world_to_base_xy(ball_pos[:2] - camera_pos[:2], base_yaw)
        local_pos = np.array([local_xy[0], local_xy[1], ball_pos[2] - camera_pos[2]])
    ...
    def update_ball_sensing(self, reset_timer, episode_step):
        ...
        completed_steps = episode_step + (0 if reset_timer else 1)
        unseen_termination_active = completed_steps >= self.initial_unseen_grace_steps
        ball_unseen_too_long = (
            self.enable_ball_unseen_termination
            and unseen_termination_active
            and (time_since_ball_seen >= self.max_ball_unseen_seconds)
        )
    ...
    def update_ball_possession_info(self, episode_step):
        ...
        after_warmup = (episode_step + 1) >= self.possession_warmup_steps
        outside_tight_box = (
        ...
        ball_task_active = self.is_ball_task_active()
        tight_possession_lost = self.enable_possession_termination and ball_task_active and after_warmup and ball_possession_armed and outside_tight_box
        immediate_possession_lost = self.enable_possession_termination and ball_task_active and ball_possession_armed and (
        ...
        self.internal_state["info"]["env_info/ball_inside_possession_pocket"] = float(inside_possession_pocket)
        self.internal_state["info"]["env_info/ball_possession_armed"] = float(ball_possession_armed)
    ...
    def update_termination_info(
        ...
        self.internal_state["info"]["env_info/termination_height"] = float(height_termination)
        self.internal_state["info"]["env_info/termination_ball_unseen"] = float(ball_unseen_too_long)
        self.internal_state["info"]["env_info/termination_tight_possession"] = float(tight_possession_lost)
        self.internal_state["info"]["env_info/termination_immediate_possession"] = float(immediate_possession_lost)
        self.internal_state["info"]["env_info/termination_qvel_limit"] = float(qvel_limit_termination)
        self.internal_state["info"]["env_info/terminated"] = float(terminated)
        self.internal_state["info"]["env_info/truncated"] = float(truncated)
        if height_termination:
            reason = 1.0
        elif ball_unseen_too_long:
            reason = 2.0
        elif tight_possession_lost:
            reason = 3.0
        elif immediate_possession_lost:
            reason = 4.0
        elif qvel_limit_termination:
            reason = 5.0
        elif truncated:
            reason = 6.0
        else:
            reason = 0.0
        self.internal_state["info"]["env_info/termination_reason"] = reason
        self.internal_state["info"]["env_info/root_qvel_norm"] = np.linalg.norm(self.internal_state["data"].qvel[:3])
        self.internal_state["info"]["env_info/root_qvel_max_abs"] = np.max(np.abs(self.internal_state["data"].qvel[:3]))
    ...
    def command_resampling_curriculum_coeff(self, level):
        if not self.command_resampling_curriculum_enabled or self.command_resampling_max_level == 0:
            return 1.0
        return float(level) / float(self.command_resampling_max_level)


    def update_command_resampling_curriculum(self, episode_step):
        level = float(self.internal_state["command_resampling_level"])
        streak = float(self.internal_state["command_resampling_streak"])

        threshold_index = min(int(level), self.command_resampling_max_level - 1)
        threshold = self.command_resampling_episode_length_thresholds[threshold_index]
        if episode_step > 0:
            if episode_step >= threshold:
                streak += 1.0
            else:
                streak = 0.0

        if streak >= self.command_resampling_successes_required and level < self.command_resampling_max_level:
            level += 1.0
            streak = 0.0

        coeff = self.command_resampling_curriculum_coeff(level)
        self.internal_state["command_resampling_level"] = level
        self.internal_state["command_resampling_streak"] = streak
        self.internal_state["command_resampling_curriculum_coeff"] = coeff
        self.internal_state["info"]["env_info/command_resampling_level"] = level
        self.internal_state["info"]["env_info/command_resampling_streak"] = streak
        self.internal_state["info"]["env_info/command_resampling_coeff"] = coeff
        self.internal_state["info"]["env_info/command_resampling_probability"] = coeff * self.command_resampling_probability_cap


    def is_ball_task_active(self):
        if not self.task_curriculum_enabled:
            return True
        return self.internal_state["locomotion_task_stage"] >= self.ball_task_stage


    def update_task_curriculum(self):
        stage = float(self.internal_state["locomotion_task_stage"])
        streak = float(self.internal_state["locomotion_task_stage_success_streak"])
        episode_step = self.internal_state["info_episode_store"]["episode_step"]
        avg_xy_velocity_diff_abs = (
            self.internal_state["info_episode_store"]["episode_total_xy_velocity_diff_abs"]
            / max(float(episode_step), 1.0)
        )

        stage_can_advance = self.task_curriculum_enabled and stage < self.ball_task_stage
        if stage_can_advance and episode_step > 0:
            episode_success = (
                episode_step >= self.ball_activation_episode_length_threshold
                and avg_xy_velocity_diff_abs <= self.ball_activation_max_xy_velocity_diff
            )
            streak = streak + 1.0 if episode_success else 0.0

        if stage_can_advance and streak >= self.ball_activation_successes_required:
            stage = self.ball_task_stage
            streak = 0.0

        self.internal_state["locomotion_task_stage"] = stage
        self.internal_state["locomotion_task_stage_success_streak"] = streak
        self.internal_state["info"]["env_info/locomotion_task_stage"] = stage
        self.internal_state["info"]["env_info/locomotion_task_stage_success_streak"] = streak
        self.internal_state["info"]["env_info/locomotion_task_ball_active"] = float(self.is_ball_task_active())
        self.internal_state["info"]["env_info/locomotion_task_avg_xy_vel_diff_abs"] = avg_xy_velocity_diff_abs
    ...
    def reset(self, seed=None):
        self.update_task_curriculum()
        self.update_command_resampling_curriculum(self.internal_state["info_episode_store"]["episode_step"])
        ...
        if episode_success:
            self.internal_state["env_curriculum_levels_in_a_row"] = (
                self.internal_state["env_curriculum_levels_in_a_row"] + 1.0
                if self.internal_state["env_curriculum_levels_in_a_row"] >= 0
                else 1.0
            )
        else:
            self.internal_state["env_curriculum_levels_in_a_row"] = (
        ...
        self.internal_state["env_curriculum_coeff"] = np.clip(
            ...
            + self.internal_state["env_curriculum_levels_in_a_row"] / self.env_curriculum_nr_levels,
            0.0,
            1.0,
        )
        ...
        self.internal_state["ball_detection_distance"] = 0.0
        self.internal_state["ball_detection_azimuth"] = 0.0
        self.internal_state["ball_detection_elevation"] = 0.0
        self.internal_state["ball_detection_local_pos"] = np.zeros(3)
        ...
        if self.command_sampling_function.setup(self.internal_state["command_resampling_curriculum_coeff"]):
    ...
    def step(self, action):
        ...
        should_sample_commands = self.command_sampling_function.step(
            self.internal_state["command_resampling_curriculum_coeff"]
        )
    ...
    def get_observation(self, action):
        ball_rel_base = self.relative_ball_position_base()
        ball_vel_base = self.ball_velocity_base()
        ball_pos_world = self.ball_position_world()
        ball_vel_world = self.ball_velocity_world()
        ball_noise = (
            self.np_rng.normal(size=3).astype(np.float32)
            * self.ball_relative_position_noise
            * (0.0 if self.internal_state["in_eval_mode"] else 1.0)
        )
        actor_ball_rel_base = ball_rel_base + ball_noise
        actor_ball_observation = np.concatenate([
            np.clip(actor_ball_rel_base / self.ball_observation_distance_scale, -1.0, 1.0),
            np.clip(ball_vel_base / self.ball_velocity_observation_scale, -1.0, 1.0),
            np.array([
                np.clip(self.internal_state["ball_detection_distance"] / self.ball_observation_distance_scale, 0.0, 1.0),
                np.clip(self.internal_state["ball_detection_azimuth"] / self.sensing_half_horizontal_range, -1.0, 1.0),
                np.clip(self.internal_state["ball_detection_elevation"] / self.sensing_half_vertical_range, -1.0, 1.0),
                float(self.internal_state["ball_visible"]) * 2.0 - 1.0,
                np.clip(self.internal_state["time_since_ball_seen"] / self.max_ball_unseen_seconds, 0.0, 1.0),
            ]),
        ])
        critic_ball_observation = np.concatenate([
            np.clip(ball_rel_base / self.ball_observation_distance_scale, -1.0, 1.0),
            np.clip(ball_vel_base / self.ball_velocity_observation_scale, -1.0, 1.0),
            np.clip(ball_pos_world / self.ball_observation_distance_scale, -10.0, 10.0),
            np.clip(ball_vel_world / self.ball_velocity_observation_scale, -1.0, 1.0),
            np.array([
                float(self.internal_state["ball_visible"]) * 2.0 - 1.0,
                np.clip(self.internal_state["time_since_ball_seen"] / self.max_ball_unseen_seconds, 0.0, 1.0),
                float(self.internal_state["ball_possession_armed"]) * 2.0 - 1.0,
            ]),
        ])
        if not self.is_ball_task_active():
            actor_ball_observation = np.zeros_like(actor_ball_observation)
            critic_ball_observation = np.zeros_like(critic_ball_observation)
    ...
    def get_observation_space(self):
        ...
        self.actor_ball_obs_idx = np.array([current_observation_idx + i for i in range(11)], dtype=int)
        current_observation_idx += 11
        self.critic_ball_obs_idx = np.array([current_observation_idx + i for i in range(15)], dtype=int)
