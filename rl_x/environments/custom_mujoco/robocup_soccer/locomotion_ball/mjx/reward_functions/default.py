...
class DefaultReward:
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        actuator_joint_nominal_diff = (
            data.qpos[self.env.actuator_joint_mask_qpos]
            - internal_state["actuator_joint_nominal_positions"]
        ) * internal_state["actuator_joint_keep_nominal"]
        actuator_joint_nominal_diff_norm = jnp.mean(jnp.square(actuator_joint_nominal_diff * self.env.actuator_joint_nominal_diff_weights))
        head_nominal_diff_norm = (
            jnp.sum(jnp.square(actuator_joint_nominal_diff) * self.env.head_actuator_joint_mask)
            / jnp.maximum(jnp.sum(self.env.head_actuator_joint_mask), 1.0)
        )
        ...
        ball_task_active = self.env.is_ball_task_active(internal_state).astype(jnp.float32)
        base_xy = self.env.base_position_world(data)[:2]
        ...
        feet_center_separation_y = jnp.abs(foot_0_base_xy[1] - foot_1_base_xy[1])
        feet_inner_gap = feet_center_separation_y - jnp.sum(self.env.foot_lateral_half_widths)
        feet_ball_gap_error = (feet_inner_gap - self.env.target_feet_inner_gap) / jnp.maximum(self.feet_ball_gap_scale, 1e-6)
        feet_ball_gap_reward = ball_task_active * curriculum_coeff * self.feet_ball_gap_coeff * -jnp.square(feet_ball_gap_error)
        ...
        ball_rel_base = self.env.relative_ball_position_base(data, internal_state)
        ball_pos_world = self.env.ball_position_world(data)
        ball_vel_world = self.env.ball_velocity_world(data)
        ball_vel_base = self.env.ball_velocity_base(data, internal_state)
        ball_attractor_error = (ball_rel_base[:2] - self.ball_attractor_target) / jnp.maximum(self.ball_attractor_scale, 1e-6)
        ball_attractor_error_norm = jnp.sum(jnp.square(ball_attractor_error))
        ...
        ball_y_too_far_side = jnp.maximum(jnp.abs(ball_rel_base[1]) - self.ball_possession_soft_max_abs_y, 0.0)
        ball_possession_violation = jnp.array([
            (ball_x_too_far_back + ball_x_too_far_front) / jnp.maximum(self.ball_possession_scale[0], 1e-6),
        ...
        info[f"env_info/head_nominal_diff_norm"] = head_nominal_diff_norm
        ...
        info[f"env_info/goal_vx"] = internal_state["goal_velocities"][0]
        info[f"env_info/goal_vy"] = internal_state["goal_velocities"][1]
        info[f"env_info/goal_wz"] = internal_state["goal_velocities"][2]
        info[f"env_info/goal_speed_xy"] = jnp.linalg.norm(internal_state["goal_velocities"][:2])
        info[f"env_info/is_standing_command"] = jnp.all(internal_state["goal_velocities"] == 0.0).astype(jnp.float32)
        info[f"env_info/ball_rel_base_x"] = ball_rel_base[0]
        ...
        info[f"env_info/ball_position_y"] = ball_pos_world[1]
        info[f"env_info/ball_attractor_error_norm"] = ball_attractor_error_norm
        info[f"env_info/ball_possession_violation_norm"] = ball_possession_violation_norm
        info[f"env_info/feet_inner_gap"] = feet_inner_gap
        info[f"env_info/feet_ball_gap_error"] = feet_inner_gap - self.env.target_feet_inner_gap
