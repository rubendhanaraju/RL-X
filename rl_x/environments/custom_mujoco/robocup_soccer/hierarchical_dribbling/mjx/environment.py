...
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.state import State
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.viewer import MujocoViewer
...
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.gait_manager_functions.handler import get_gait_manager_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robocup_soccer.hierarchical_dribbling.mjx.terrain_functions.handler import get_terrain_function
...
class HierarchicalDribblingEnv:
    def __init__(self, robot_config, runner_mode, render, env_config, nr_envs):
        ...
        self.single_action_space = BoxSpace(
            low=jnp.concatenate([command_low, residual_low]),
            high=jnp.concatenate([command_high, residual_high]),
            shape=(action_space_size,),
            dtype=jnp.float32,
            center=jnp.concatenate([jnp.zeros(3, dtype=jnp.float32), nominal_joint_positions]),
            scale=jnp.concatenate([jnp.ones(3, dtype=jnp.float32), action_scale_factor * jnp.ones(self.nr_actuator_joints, dtype=jnp.float32)]),
        )
    ...
    def _reset(self, state):
        key, initial_state_key, terrain_key, domain_randomization_key, observation_key, gait_manager_key, ball_reset_key, ball_command_key = jax.random.split(state.key, 8)
        state = state.replace(key=key)

        state.internal_state["env_curriculum_coeff"] = 1.0
        state.internal_state["env_curriculum_levels_in_a_row"] = 0.0
    ...
    def _step(self, state, action):
        ...
        state.internal_state["actuator_joint_keep_nominal"] = actuator_joint_keep_nominal
        ...
        base_action_mean, _, next_base_policy_gru_carry = self.base_policy.apply(
            ...
            state.internal_state["base_policy_gru_carry"][None, :],
            method=self.base_policy.apply_one_step,
        )
        base_action = self.base_get_processed_action(base_action_mean)[0]
        residual_action = jnp.clip(raw_residual_action, -self.residual_action_clip, self.residual_action_clip)
        chosen_action = base_action + residual_action
        state.internal_state["base_policy_action"] = base_action
        state.internal_state["base_policy_gru_carry"] = next_base_policy_gru_carry[0]
        state.internal_state["current_residual_action"] = residual_action
    ...
    def get_observation(self, data, mjx_model, internal_state, key, action):
        ...
        observation = jnp.concatenate([
            ...
            internal_state["base_policy_action"],
            internal_state["last_residual_action"],
        ])
        ...
        observation = observation.at[self.base_policy_action_obs_idx].set(jnp.clip(observation[self.base_policy_action_obs_idx] / 10.0, -1.0, 1.0))
        observation = observation.at[self.last_residual_action_obs_idx].set(jnp.clip(observation[self.last_residual_action_obs_idx] / self.residual_action_clip, -1.0, 1.0))
    ...
    def get_observation_space(self):
        ...
        current_observation_idx += 1
        self.base_policy_action_obs_idx = jnp.array([current_observation_idx + i for i in range(self.nr_actuator_joints)])
        current_observation_idx += self.nr_actuator_joints
        self.last_residual_action_obs_idx = jnp.array([current_observation_idx + i for i in range(self.nr_actuator_joints)])
        current_observation_idx += self.nr_actuator_joints
