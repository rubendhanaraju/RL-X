...
class DefaultReward:
    ...
    def reward_yaw_alignment(self, base_yaw, base_xy, ball_xy, ball_velocity_command_xy):
        ...
        cmd_yaw = np.arctan2(ball_velocity_command_xy[1], ball_velocity_command_xy[0]) if cmd_speed > 1e-6 else base_yaw
    ...
    def reward_and_info(self, action):
        ...
        ball_velocity_command = self.env.internal_state["ball_velocity_command"]
        ball_velocity_tracking_error = np.sum(np.square(ball_vel_world[:2] - ball_velocity_command))
        ...
        residual_action_delta = residual_action - self.env.internal_state["last_residual_action"]
        residual_action_norm = np.sum(np.square(residual_action) * self.env.residual_action_l2_mask)
        residual_action_smoothness_norm = np.sum(np.square(residual_action_delta) * self.env.residual_action_smoothness_mask)
        residual_action_head_norm = np.sum(np.square(residual_action) * self.env.residual_action_head_mask)
        residual_action_non_head_norm = np.sum(np.square(residual_action) * self.env.residual_action_non_head_mask)
        residual_action_head_smoothness_norm = np.sum(np.square(residual_action_delta) * self.env.residual_action_head_mask)
        residual_action_non_head_smoothness_norm = np.sum(np.square(residual_action_delta) * self.env.residual_action_non_head_mask)
        ...
        self.env.internal_state["info"][f"reward/ball_velocity_tracking"] = ball_velocity_tracking_reward
        ...
        self.env.internal_state["info"][f"env_info/residual_action_non_head_norm"] = np.sqrt(residual_action_non_head_norm)
        self.env.internal_state["info"][f"env_info/residual_action_head_smoothness_norm"] = np.sqrt(residual_action_head_smoothness_norm)
        self.env.internal_state["info"][f"env_info/residual_action_non_head_smoothness_norm"] = np.sqrt(residual_action_non_head_smoothness_norm)
