...
class PPO:
    ...
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.reward_group_names = tuple(config.algorithm.reward_group_names)
        self.reward_group_weights = jnp.array(
            config.algorithm.reward_group_weights,
            dtype=jnp.float32,
        )
        self.nr_reward_groups = len(self.reward_group_names)
        ...
        self.policy_state = TrainState.create(apply_fn=self.policy.apply,
        ...
        self.critic_state = TrainState.create(apply_fn=self.critic.apply,
            ...
            def multi_learning_and_eval_save_iteration(multi_learning_and_eval_save_iteration_carry,
                ...
                def learning_iteration(learning_iteration_carry, learning_iteration_step):
                    ...
                    def single_rollout(single_rollout_carry, _):
                        ...
                        log_prob = (-0.5 * ((action - action_mean) / action_std)**2 - 0.5 * jnp.log(2.0 * jnp.pi) -
                                    action_logstd).sum(1)
                        processed_action = self.get_processed_action(action)
                        value = self.critic.apply(critic_state.params, observation)

                        env_state = self.train_env.step(env_state, processed_action)

                        reward_groups = jnp.stack(
                            [env_state.info[f"reward/{name}"] for name in self.reward_group_names],
                            axis=-1,
                        )
                    ...
                    def calculate_gae_advantages(critic_state, next_states, rewards, values, terminations):
                        next_values = self.critic.apply(critic_state.params, next_states)

                        not_done = 1.0 - terminations[..., None]

                        delta = rewards + self.gamma * next_values * not_done - values

                        def compute_advantages(carry, t):
                            prev_advantage = carry
                            advantage = (delta[t] + self.gamma * self.gae_lambda * not_done[t] * prev_advantage)
                        ...
                        advantages = jnp.concatenate(
                            [advantages[::-1], init_advantages[None, ...]],
                            axis=0,
                        )
                    ...
                    def loss_fn(policy_params, critic_params, state_b, action_b, log_prob_b, return_b, advantage_b):
                        ...
                        action_std = jnp.exp(action_logstd)
                        new_log_prob = -0.5 * (
                            (action_b - action_mean) / action_std)**2 - 0.5 * jnp.log(2.0 * jnp.pi) - action_logstd
                        ...
                        critic_loss_by_group = 0.5 * (new_value - return_b)**2
                        critic_loss = jnp.mean(critic_loss_by_group)
                    ...
                    batch_advantages = advantages.reshape((-1, self.nr_reward_groups))
                    batch_returns = returns.reshape((-1, self.nr_reward_groups))
                    ...
                    def minibatch_update(carry, minibatch_indices):
                        ...
                        minibatch_advantages_by_group = batch_advantages[minibatch_indices]

                        minibatch_advantages_by_group = (minibatch_advantages_by_group - jnp.mean(
                            minibatch_advantages_by_group, axis=0, keepdims=True)) / (
                                jnp.std(minibatch_advantages_by_group, axis=0, keepdims=True) + 1e-8)

                        minibatch_advantages = jnp.sum(
                            minibatch_advantages_by_group * self.reward_group_weights[None, :],
                            axis=-1,
                        )
                        (loss, (metrics)), (policy_gradients, critic_gradients) = grad_loss_fn(
                            policy_state.params, critic_state.params, batch_states[minibatch_indices],
                            batch_actions[minibatch_indices], batch_log_probs[minibatch_indices],
                            batch_returns[minibatch_indices], minibatch_advantages)
                    ...
                    explained_variance_by_group = 1.0 - (jnp.var(returns - values, axis=(0, 1)) /
                                                         (jnp.var(returns, axis=(0, 1)) + 1e-8))

                    optimization_metrics["v_value/explained_variance"] = jnp.mean(explained_variance_by_group)
                ...
                if self.evaluation_active:
                    ...
                    combined_learning_iteration_step = (multi_learning_iteration_step +
    ...
    def save(self, policy_state, critic_state):
        checkpoint = {"policy": policy_state, "critic": critic_state}
