...
Output:
  ./tmp/target_vs_mixture_samples.png
...
def save_outputs(state, key, args):
    out_dir = Path.cwd() / "tmp"
    out_dir.mkdir(parents=True, exist_ok=True)

    sample_key = key
    samples = np.asarray(sample_terminal_gmm(state.params, sample_key, args.n_plot_samples, args.var_floor))
    ...
    axes[1].scatter(samples[:, 0], samples[:, 1], s=2, alpha=0.25)
    axes[1].scatter(means_np[:, 0], means_np[:, 1], s=80, marker="x")
    axes[1].set_xlim(-lim, lim)
    axes[1].set_ylim(-lim, lim)
    axes[1].set_aspect("equal")
    axes[1].set_title("Samples from learned terminal GMM $q_T$")
...
def main():
    ...
    save_outputs(state, key_plot, args)
