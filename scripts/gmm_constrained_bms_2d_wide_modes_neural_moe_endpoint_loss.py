...
This variant uses a neural mixture-of-experts control plus an explicit
endpoint forward-KL / weighted maximum-likelihood loss for the terminal GMM:
  ...
  - the terminal GMM itself is trained with SNIS weights rho_target(x)/q_old(x).
...
def gmm_log_prob_from_params(params, x: jnp.ndarray, var_floor: float) -> jnp.ndarray:
    """
    log q_phi,T(x) for the explicit terminal diagonal GMM.
    ...
    log_comp = (
        log_pi[None, :]
        - 0.5 * jnp.sum(jnp.log(2.0 * jnp.pi * vars_T[None, :, :]) + diff**2 / vars_T[None, :, :], axis=-1)
    )
    return jax.nn.logsumexp(log_comp, axis=-1)


def snis_weights_from_endpoint_samples(params, xT: jnp.ndarray, var_floor: float):
    """
    Self-normalized endpoint importance weights for expectations under p_target.

    Samples xT are drawn from q_old = q_{params,T}. Since p_target = rho/Z,
    the unknown Z cancels:
        w_i ∝ rho(xT_i) / q_old(xT_i).
    """
    log_q_old = gmm_log_prob_from_params(params, xT, var_floor)
    log_w = target_log_rho(xT) - log_q_old
    log_w = jax.lax.stop_gradient(log_w)
    w = jax.nn.softmax(log_w, axis=0)
    ess = 1.0 / jnp.sum(w**2)
    return w, ess, jnp.max(log_w), jnp.std(log_w)
...
def train_step(
    ...
    # SNIS endpoint weights. These are the key fix: the terminal GMM parameters
    # now receive a direct forward-KL / weighted maximum-likelihood signal.
    endpoint_w, ess, max_logw, std_logw = snis_weights_from_endpoint_samples(
        state.params, xT, var_floor
    )
    ...
    def loss_fn(params):
        ...
        per_ex_drift = jnp.sum((u_pred - xi)**2, axis=-1)
        per_ex_damp = jnp.sum((u_pred - u_ref)**2, axis=-1)

        # Weight the BMS regression toward the true endpoint target distribution.
        drift_mse = 0.5 * jnp.sum(endpoint_w * per_ex_drift)
        damping = 0.5 * eta * jnp.sum(endpoint_w * per_ex_damp)

        logits, means_T, vars_T = terminal_params_from_flax_params(params, var_floor)
        pi = jax.nn.softmax(logits)

        # Explicit forward-KL terminal projection term:
        #   min_phi KL(p_target || q_phi,T)
        # is equivalent to max_phi E_{p_target}[log q_phi,T(X)].
        # We estimate that expectation with SNIS samples from q_old.
        new_log_q = gmm_log_prob_from_params(params, xT, var_floor)
        endpoint_forward_kl_loss = -endpoint_kl_coef * jnp.sum(endpoint_w * new_log_q)
        ...
        total = drift_mse + damping + endpoint_forward_kl_loss + entropy_loss + var_loss

        metrics = {
            "loss": total,
            "drift_mse": drift_mse,
            "damping": damping,
            "endpoint_forward_kl_loss": endpoint_forward_kl_loss,
            "ess": ess,
            "max_logw": max_logw,
            "std_logw": std_logw,
...
def parse_args():
    ...
    parser.add_argument("--endpoint_kl_coef", type=float, default=2.0)
...
def main():
    ...
    for step in range(1, args.steps + 1):
        ...
        state, metrics = train_step(
            ...
            endpoint_kl_coef=args.endpoint_kl_coef,
        )
        ...
        if step == 1 or step % args.log_every == 0:
            ...
            print(f"step {step:6d} | "
                  ...
                  f"end {float(m['endpoint_forward_kl_loss']):9.4f} | "
                  f"ESS {float(m['ess']):7.1f} | "
                  f"logw_sd {float(m['std_logw']):7.2f} | "
