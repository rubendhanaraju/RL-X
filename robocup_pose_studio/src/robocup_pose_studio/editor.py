...
from robocup_pose_studio.viewer import MujocoViewer
...
def resolve_default_xml_path():
    return Path(__file__).resolve().parent / "assets" / "plane.xml"
...
class PoseStudio:
    def __init__(self, xml_path, load_path=None):
        ...
        self.root = tk.Tk()
        self.root.title("RoboCup Pose Studio")
    ...
    def _build_ui(self):
        ...
        tk.Button(
            ...
            text="Move Up",
            command=lambda: self._move_keyframe(-1),
        ).pack(side="left", padx=4)
        tk.Button(
            timeline_actions_bottom,
            text="Move Down",
        ...
        self.keyframe_listbox = tk.Listbox(
            timeline_frame, height=7, exportselection=False
        )
    ...
    def _apply_pose_to_sim(self):
        qpos = (
            self.playback_qpos if self.playback_qpos is not None else self._current_qpos()
        )
        self.data.qpos = qpos
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Standalone pose and keyframe editor for the RoboCup MuJoCo robot."
    )
    parser.add_argument(
        "--xml",
        type=Path,
        default=resolve_default_xml_path(),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional JSON pose or animation file to load on startup.",
    )
...
def main():
    args = parse_args()
    studio = PoseStudio(xml_path=args.xml, load_path=args.load)
    studio.run()
