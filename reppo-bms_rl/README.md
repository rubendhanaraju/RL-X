...
If you want to use the humanoid_bench environments, then first pull the dependency
```bash
git submodule update --init --recursive
```
and then sync with the extra dependencies
```bash
uv sync --extra humanoid-bench
```
