...
class DiffusionModel(nnx.Module):
    def __init__(
        ...
        forward_integrated_scheduler: callable = None,
        delta_integrated_scheduler: callable = None,
        backward_integrated_scheduler: callable = None,
        *,
        rngs: nnx.Rngs,
    ):
        ...
        self.forward_integrated_scheduler = forward_integrated_scheduler
        self.delta_integrated_scheduler = delta_integrated_scheduler
        self.backward_integrated_scheduler = backward_integrated_scheduler
        
        # Learnable parameters (converted from the params dict)
        self.betas = nnx.Param(jnp.ones((diff_steps,)))
        self.prior_mean = nnx.Param(jnp.zeros((action_dim,))) if learn_prior else jnp.zeros((action_dim,))
        self.prior_std = nnx.Param(jnp.ones((action_dim,)) * inverse_softplus(init_std)) if learn_prior else jnp.ones((action_dim, )) * self.init_std
        self.mass_std = nnx.Param(jnp.ones(1) * inverse_softplus(1.0))

        dt = 1. / diff_steps
        self.alpha_vals = 1 - jnp.exp(
            -2
            * jnp.array(
                [
                    delta_integrated_scheduler(step * dt, (step + 1) * dt)
                    for step in range(diff_steps + 1)
                ]
            )
        )
        ...
        def _alpha(step):
            ...
            return self.alpha_vals[idx]

        @jax.jit
        def _kappa(step):
            return (self.init_std * (1 - jnp.sqrt(1 - _alpha(step)))) ** 2 / _alpha(step)
        
        @jax.jit
        def _b(step, x):
            beta = self.scheduler(step * dt)
            return - beta * x

        @jax.jit
        def _nabla_b(step, x):
            beta = self.scheduler(step * dt)
            return -beta * jnp.eye(action_dim)
        ...
        self.kappa = _kappa
        self.b = _b
        self.nabla_b = _nabla_b
        self.f = f
        self.nabla_f = _nabla_f
...
class DIMEActor(nnx.Module):
    ...
    def sde_sample(
        ...
        def _sde_sample(key, obs):
            ...
            terminal_costs = self.diffusion_model.prior_log_prob(final_x)
