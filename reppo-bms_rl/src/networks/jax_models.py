...
from src.jaxrl.lagrangian_utils import tr_dual_fn, compute_tr_lm, compute_ent_lm, ent_dual_fn
...
class SACActorNetworks2(nnx.Module):
    def __init__(
        self,
        obs_dim: int,
        action_dim: int,
        ...
        *,
        ...
        start_value = ent_start
        kl_start_value = math.log(kl_start)
        self.temperature_val = nnx.Param(jnp.ones(1) * start_value)
        self.lagrangian_log_param = nnx.Param(jnp.ones(1) * kl_start_value)
        self.min_std = min_std


        tr_dual = partial(tr_dual_fn, tr_bound=0.1)
        self.optimize_tr_lm = jax.jit(partial(compute_tr_lm, dual_fn=tr_dual))
        ent_dual = partial(ent_dual_fn, ent_bound=entropy_target)
        self.optimize_ent_lm = jax.jit(partial(compute_ent_lm, dual_fn=ent_dual))
    ...
    # --- GETTER ---
    def temperature(self) -> jax.Array:
        """Returns the actual temperature value (exp(log_param))."""
        return self.temperature_val.value

    # --- SETTER (New) ---
    def set_temperature(self, temp: float | jax.Array):
        """
        Sets the temperature.
        Automatically converts to log space and ensures shape consistency.
        """
        # 1. Convert input to JAX array if it's a float
        val = jnp.array(temp)

        # 3. Ensure shape matches the parameter initialization (usually (1,))
        if val.ndim == 0:
            val = jnp.expand_dims(val, 0)

        # 4. Update the NNX Param value directly
        self.temperature_val.value = val
