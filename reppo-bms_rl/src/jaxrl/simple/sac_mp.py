...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            train_state, actor_state, critic_states, buffer_state, env_state, last_obs, rng = runner_state

            # STEP THE ENV
            rng, rng_a, rng_s = jax.random.split(rng, 3)
            action = sac_exploration(
                rng_a, actor_state, last_obs, train_state.timesteps
            )
            ...
            # SAC NETWORKS UPDATE
            def _learn_phase(train_state, actor_state, critic_states, rng):
                # Define number of gradient steps per learning phase
                ...
                rng_keys = jax.random.split(rng, gradient_steps)
                ...
                return final_train_state, final_actor_state, final_critic_states, final_losses
            ...
            train_state, actor_state, critic_states, losses = jax.lax.cond(
                is_learn_time,
                lambda operand, rng: _learn_phase(operand[0], operand[1], operand[2], rng),
                lambda operand, rng: (operand[0], operand[1], operand[2], {
                    "critic_loss": jnp.array(0.0),
                    "actor_loss": jnp.array(0.0),
                    "entropy_loss": jnp.array(0.0),
                    "alpha": jnp.exp(train_state.entropy_coef.params["log_alpha"]),
                    "q_vals": jnp.array(0.0)
                }),
                (train_state, actor_state, critic_states),
                _rng,
            )
            ...
            runner_state = (train_state, actor_state, critic_states, buffer_state, env_state, obs, rng)
            return runner_state, metrics

        # train
        rng, _rng = jax.random.split(rng)
        runner_state = (train_state, actor_train_state, critic_states, buffer_state, env_state, init_obs, _rng)
