...
from src.networks.jax_models import (
    ...
    CriticNetwork,
    SACActorNetworks2, SACActorNetworks,
)
...
def make_train_fn(
    ...
    def collect_rollout(
        ...
        def step_env(carry, _) -> tuple[tuple, Transition]:
            ...
            behavior_log_prob = pi.log_prob(action).sum(-1)
            raw_importance_weight = jnp.nan_to_num(
                og_pi.log_prob(action).sum(-1) - behavior_log_prob,
    ...
    def learn_step(
        ...
        def reweight_with_critic(train_state: SACTrainState, data: Transition) -> Transition:
            """
            ...
            # Define a function to compute Q for a single sample (or rely on vmap)
            def get_q_value(c_obs, act):
                # Assuming forward returns: (emb, pred, pred_rew, value)
                # We want the last output: value (Q-value)
                Q_value = critic_model.critic(c_obs, act)
                return jax.lax.stop_gradient(Q_value)

            # If your critic handles the full batch size without OOM, call directly.
            # Otherwise, use vmap.
            # Note: .squeeze() is usually needed as Q is often (Batch, 1)
            q_values = jax.vmap(get_q_value)(data[0].critic_obs, data[0].action).squeeze()

            # Calculate new weights in log space
            # Formula: log( exp(Q) / pi ) = Q - log(pi)
            new_weights = q_values - data[0].log_weight

            alpha_star, error = find_alpha_bisection(q_values, data[0].log_weight, -action_size_target)

            tempered_log_w = alpha_star * q_values - data[0].log_weight
            tempered_log_Z = jax.scipy.special.logsumexp(tempered_log_w) - jnp.log(tempered_log_w.shape[0])

            # Normalized weights for the tempered distribution
            tempered_log_norm_weights = tempered_log_w - jax.scipy.special.logsumexp(tempered_log_w)
            tempered_normalized_weights = jnp.exp(tempered_log_norm_weights)

            tempered_Q_log_probs = alpha_star * q_values - tempered_log_Z

            # Entropy of the tempered distribution
            tempered_entropy = -jnp.sum(tempered_normalized_weights * tempered_Q_log_probs)

            jax.debug.print("Found Alpha: {}", alpha_star)
            jax.debug.print("Error: {}", error)

            jax.debug.print("Temperature: {}", 1. / alpha_star)
            jax.debug.print("Tempered Entropy: {}", tempered_entropy)

            # Optional: Clip weights if necessary to prevent numerical explosion
            # new_weights = jnp.clip(new_weights, min=jnp.log(cfg.lmbda_min), max=jnp.log(50.0))

            return data[0].replace(log_weight=new_weights), 1. / alpha_star
        ...
        # new_data, Q_temperature = reweight_with_critic(train_state, data)
        # data = (new_data, data[1])

        # actor_model = nnx.merge(train_state.actor.graphdef, train_state.actor.params)
        # actor_model.set_temperature(Q_temperature)
