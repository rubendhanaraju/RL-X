        ...
        def update_actor(train_state, key) -> tuple[SACTrainState, dict[str, jax.Array]]:
            def minibatch_update(carry, indices):
                ...
                def adjoint_matching(params):
                    ...
                    key_t, key_noise, key_t_ent, key_ent = jax.random.split(key, 4)  # Split the key passed to actor_loss
                    ...
                    # kappa = (old_entropy - H_0) * 0.9 + H_0
                    #
                    # lm_tr, lm_entr, lm_ent_lb, dual_val, state = actor_model.optimize_lm(Q_vals=Q_value,
                    ...
                    #             ((lm_entr + lm_ent_lb) / (lm_tr + lm_entr + lm_ent_lb)) * log_weights
                    #             + (1.0/ (lm_tr + lm_entr + lm_ent_lb)) * Q_value.reshape(log_weights.shape))
                    # self_normalized_weights = jnp.exp(
                    #     smoothed_log_importance_weights - logsumexp(smoothed_log_importance_weights))

                    # smoothed_log_importance_weights = (1.0 / (lm_entr + lm_ent_lb)) * Q_value.reshape(log_weights.shape)
                    # self_normalized_weights = jnp.exp(
                    #     smoothed_log_importance_weights - logsumexp(smoothed_log_importance_weights))

                    # # actor_model.set_fixed_temperature(lm_entr + lm_ent_lb + 1e-6)
                    # entr_coeff = infer_entr_coeff_via_is(log_weights, Q_value, -17.0)
                    # actor_model.set_fixed_temperature(entr_coeff + 1.0e-6)
                    # # actor_model.set_fixed_temperature(0.001)
                    #
                    ...
                    # adjoint_state = (nabla_p_T_ref - tanh_correction_grad) - (Q_score / jax.lax.stop_gradient(actor_model.temperature()))
                    ...
                    # adjoint_state = scaler*(0 - tanh_correction_grad) - Q_score
                    # adjoint_loss = 0.5 * jnp.sum(jnp.square(scaler*ctrl + sigma_t * adjoint_state), axis=-1)
                    ...
                    # log_i_weights = log_weights.squeeze() + Q_value / jax.lax.stop_gradient(actor_model.temperature())
                    # ni_weights = jax.lax.stop_gradient(jax.nn.softmax(log_i_weights.squeeze()))
                    # ni_weights = ni_weights.reshape(log_weights.shape)

                    # i_weights = -jnp.sum(jnp.square(ctrl), axis=-1) - Q_value / jax.lax.stop_gradient(
                    #     actor_model.temperature()) - tanh_correction_val
                    # ni_weights = jax.lax.stop_gradient(i_weights / (jnp.sum(i_weights) + 1e-6))
                    # ni_weights = ni_weights.reshape(adjoint_state.shape)
                    # adjoint_loss = 0.5 * jnp.sum(ni_weights*jnp.square(ctrl + sigma_t * adjoint_state), axis=-1)
                    ...
                    # adjoint_loss = adjoint_loss * sigma_t.reshape(adjoint_loss.shape)**(-1)
                    ...
                    n_samples = 1
                    t_ent = jax.random.uniform(key_t_ent, (batch_size, n_samples, 1))
                    flat_t_ent = t_ent.reshape((batch_size*n_samples,1))
                    action_dim = a_T_unsquashed.shape[1]

                    # 2. Randomly sample noise from N(0, I)
                    noise_ent = jax.random.normal(key_ent, (batch_size, n_samples, action_dim))

                    # Retrieve coefficients from scheduler (handling broadcasting)
                    mu_scale_ent = scheduler.mu_t_0T_scale(flat_t_ent).reshape((batch_size, n_samples, 1))  # Shape: (Batch*n_samples, 1)
                    sigma_scale_ent = scheduler.sigma_t_0T(flat_t_ent).reshape((batch_size, n_samples, 1))  # Shape: (Batch, 1)
                    sigma_t_ent = scheduler.sigma_t(flat_t_ent).reshape((batch_size, n_samples, 1))  # Shape: (Batch, 1)

                    # a_t = mu(t) * a_T + sigma(t) * noise
                    a_t_ent = mu_scale_ent * a_T_unsquashed.reshape((batch_size, 1,action_dim)) + sigma_scale_ent * noise_ent

                    # a_t_ent: (batch, n_samples, action_dim) -> (batch*n_samples, action_dim)
                    flat_a_t_ent = a_t_ent.reshape((batch_size * n_samples, action_dim))

                    # obs: (batch, obs_dim) -> (batch*n_samples, obs_dim)  via repeat
                    flat_obs_ent = jnp.repeat(obs, n_samples, axis=0)  # repeat each obs n_samples times and put in flattened shape

                    flat_scores = jax.vmap(diffusion.fwd_model, in_axes=(0, 0, 0))(flat_a_t_ent, flat_obs_ent, flat_t_ent)

                    # reshape back: (batch*n_samples, action_dim) -> (batch, n_samples, action_dim)
                    ctrl_ent = sigma_t_ent * flat_scores.reshape((batch_size, n_samples, action_dim))

                    det = -0.5*jnp.sum(ctrl_ent**2, axis=-1) # sum over dimensions
                    # stoch = -jnp.sum(ctrl_ent*noise_ent, axis=-1) # sum over dimensions # Checkout MC approx of stochastic integral
                    # drift_coeff = scheduler.sigma_t(flat_t_ent) / (scheduler.sigma_t_T(flat_t_ent) ** 2)
                    drift_coeff = scheduler.sigma_t(flat_t_ent) / jnp.clip(scheduler.sigma_t_T(flat_t_ent) ** 2, min=1e-6)
                    drift_coeff = drift_coeff.reshape((batch_size, n_samples, 1))
                    stoch = - jnp.sum(ctrl_ent * drift_coeff *(a_T_unsquashed.reshape((batch_size, 1,action_dim))-a_t_ent), axis=-1)
                    entropy = jnp.sum(det + stoch, axis=-1) + tanh_correction_val.squeeze() # sum over the trajectories gives entropy for (batch)
                    entropy = (1/n_samples)*entropy

                    entropy = (1/n_samples)*(det + tanh_correction_val.squeeze())




                    optimal_entropy = compute_entropy_via_importance_sampling(jax.lax.stop_gradient(log_weights),
                                                                              jax.lax.stop_gradient(Q_value),
