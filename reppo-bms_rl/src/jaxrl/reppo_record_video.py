...
def make_train_fn(
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state):
            ...
            return train_state, metrics, all_rollout_actions, eval_actions, trajectory_states  # Return both rollout and eval actions

        def loop_body(
            train_state: SACTrainState, key: PRNGKey
        ) -> tuple[SACTrainState, dict]:
            key, subkey = jax.random.split(key)
            train_state, metrics, all_rollout_actions, eval_actions, trajectory_states = jax.vmap(train_eval_step)(
        ...
        key, init_key = jax.random.split(key)
        train_state = jax.vmap(make_init(cfg, env, env_render, env_params, env_render_params))(
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    train_fn = make_train_fn(
        ...
        env=env,
        env_render=env_render if cfg.hyperparameters.get("render_interval", 0) > 0 else None,
