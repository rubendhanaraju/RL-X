...
def make_init(
    ...
    def init(key: jax.random.PRNGKey):
        ...
        ), actor_state, ent_coef_state, critic_states
...
def make_train_fn(
    ...
    def learn_step(
        key: PRNGKey, train_state: SACTrainState, actor_state: TrainState, ent_coef_state: TrainState, critic_states: TrainState
    ) -> tuple[SACTrainState, dict[str, jax.Array]]:

        def update(key: PRNGKey, train_state: SACTrainState, actor_state: TrainState, ent_coef_state: TrainState, critic_states: TrainState) -> tuple[SACTrainState, dict[str, jax.Array]]:
            def minibatch_update(carry, rng_key):
                idx, train_state, actor_state, ent_coef_state, critic_states = carry
                ...
                def actor_loss(params, batch_stats, ent_coef_state): 
                ...
                (loss, (critic_state_updates, critic_metrics)), critic_grads = critic_grad_fn(critic_states.params, critic_states.batch_stats, dropout_key)
                ...
                return (idx + 1, train_state, actor_state, ent_coef_state, critic_states), {
                    **critic_metrics,
                    **actor_metrics,
                }
            ...
            (idx, train_state, actor_state, ent_coef_state, critic_states), metrics = jax.lax.scan(
                minibatch_update, (0, train_state, actor_state, ent_coef_state, critic_states), utd_keys
            )
            # Compute mean metrics across mini-batches
            metrics = jax.tree.map(lambda x: x.mean(0), metrics)
            return idx, train_state, actor_state, ent_coef_state, critic_states, metrics

        # Update the model for a number of epochs
        key, train_key = jax.random.split(key)
        _, train_state, actor_state, ent_coef_state, critic_states, update_metrics = update(
            train_key, train_state, actor_state, ent_coef_state, critic_states
        )

        return train_state, actor_state, ent_coef_state, critic_states, update_metrics

    def train_fn(key: PRNGKey, cfg: SACConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state, actor_state, ent_coef_state, critic_states):
            def train_step(
                carry, key: PRNGKey
            ) -> tuple[tuple[SACTrainState, TrainState, TrainState], dict[str, jax.Array]]:
                state, actor_state, ent_coef_state, critic_states = carry
                key, rollout_key, learn_key = jax.random.split(key, 3)
                (state, actor_state) = collect_rollout(key=rollout_key, train_state=state, actor_state=actor_state)
                state, actor_state, ent_coef_state, critic_states, update_metrics = learn_step(
                ...
                return (state, actor_state, ent_coef_state, critic_states), metrics

            train_key, eval_key = jax.random.split(key)
            eval_interval = int(
                (cfg.total_time_steps / (cfg.num_envs)) // cfg.num_eval
            )
            (train_state, actor_state, ent_coef_state, critic_states), train_metrics = jax.lax.scan(
                f=train_step,
                init=(train_state, actor_state, ent_coef_state, critic_states),
            ...
            return (train_state, actor_state, ent_coef_state, critic_states), metrics
        ...
        def loop_body(
            carry: tuple[SACTrainState, TrainState, TrainState, TrainState], key: PRNGKey
        ) -> tuple[tuple[SACTrainState, TrainState, TrainState, TrainState], dict]:
            train_state, actor_state, ent_coef_state, critic_states = carry
            key, subkey = jax.random.split(key)
            (train_state, actor_state, ent_coef_state, critic_states), metrics = jax.vmap(train_eval_step)(
                jax.random.split(subkey, num_seeds), train_state, actor_state, ent_coef_state, critic_states
            )
        ...
        key, init_key = jax.random.split(key)
        train_state, actor_state, ent_coef_state, critic_states = jax.vmap(make_init(cfg, buffer, env, env_params))(
        ...
        state, metrics = jax.lax.scan(f=loop_body, init=(train_state, actor_state, ent_coef_state, critic_states), xs=keys)
