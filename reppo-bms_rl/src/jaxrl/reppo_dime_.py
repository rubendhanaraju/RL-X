...
#     final_action, final_log_ratios = kl_diver(
#         key, actor_state, old_actor_state, actor_params, old_actor_params, obs_reshaped, stop_grad=stop_grad
#     )
#     # final_action, final_log_ratios = batched_kl_div(keys, obs_reshaped)
    ...
    def learn_step(
        ...
        def update(carry, key) -> tuple[SACTrainState, dict[str, jax.Array]]:
            ...
            def minibatch_update(carry, indices, sampler, kl_diver):
                ...
                # Simplified: Always update actor since actor_update_freq = 1.0
                ...
                def actor_loss(params, ent_params, kl_params):
                    ...
                    if cfg.dime_loss == "reverse_kl":
                        ...
                        # log
                        log_abs_pred_action = jnp.abs(pred_action).mean()
                        log_abs_batch_action = jnp.abs(minibatch.action).mean()
                    ...
                    return loss, dict(
                        ...
                        loss=loss,
                        temp=ent_coef,
                        abs_batch_action=log_abs_batch_action,
                        abs_pred_action=log_abs_pred_action,
                        reward_mean=minibatch.reward.mean(),
                        kl=log_kl,
                        lagrangian=kl_coef,
                ...
                # Direct actor update (simplified since actor_update_freq = 1.0)
                ...
                output, (actor_grads, ent_grads, kl_grads) = actor_grad_fn(
                    train_actor_state.params, train_ent_coef_state.params, train_kl_coef_state.params
                )
                
                # Apply gradients
                train_actor_state = train_actor_state.apply_gradients(grads=actor_grads)
                train_ent_coef_state = train_ent_coef_state.apply_gradients(grads=ent_grads)
                train_kl_coef_state = train_kl_coef_state.apply_gradients(grads=kl_grads)
                
                # Get actor metrics
                actor_metrics = output[1]
                actor_gnorm = utils.tree_norm(actor_grads)
                actor_metrics = {**actor_metrics, "actor_gnorm": actor_gnorm}
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state, train_actor_state, train_actor_target_state, train_ent_coef_state, train_kl_coef_state, sampler=None, kl_diver=None):
            ...
            for metric_name, value in temp_eval_metrics.items():
                ...
                # duplicate
                eval_metrics[f"{metric_name}"] = value
        ...
        def loop_body(
            ...
            # Pass all training states to the callback for checkpointing
            jax.debug.callback(log_callback, train_state, train_actor_state, train_actor_target_state, train_ent_coef_state, train_kl_coef_state, metrics)
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    def log_callback(state, actor_state, actor_target_state, ent_coef_state, kl_coef_state, metrics):
        ...
        if cfg.hyperparameters.save_checkpoints:
            # Use relative path within the current working directory (Hydra sets this automatically)
            checkpoint_dir = os.path.abspath(cfg.hyperparameters.checkpoint_dir)
            os.makedirs(checkpoint_dir, exist_ok=True)
            
            step = int(state.time_steps[0])
            
            # Create comprehensive checkpoint with all training states
            # Extract only the critic from the main state to avoid env_state issues
            critic_only_state = {
                'critic': state.critic,
                # 'last_env_state': state.last_env_state,
                'iteration': state.iteration,
                'time_steps': state.time_steps
            }
            
            # Combine all training components into a single checkpoint
            checkpoint_target = {
                'state': critic_only_state,
                'actor_state': actor_state,
                'actor_target_state': actor_target_state,
                'ent_coef_state': ent_coef_state,
                'kl_coef_state': kl_coef_state,
                'step': step
            }
            
            # Determine if we should upload to W&B
            should_upload = (
                cfg.hyperparameters.checkpoint_upload_to_wandb and 
                (len(metric_history) % cfg.hyperparameters.checkpoint_upload_freq == 0)
            )
            
            if cfg.hyperparameters.save_last_only:
                # Save only the latest checkpoint (overwrite previous)
                checkpoint_path = os.path.join(checkpoint_dir, f"{cfg.hyperparameters.checkpoint_prefix}_latest")
                checkpoints.save_checkpoint(
                    ckpt_dir=checkpoint_path,
                    target=checkpoint_target,
                    step=step,
                    overwrite=True,
                    keep=1  # Keep only the latest checkpoint
                )
                
                # Upload to wandb as artifact (overwrite existing) only if needed
                if should_upload:
                    checkpoint_artifact = wandb.Artifact(
                        name=f"{cfg.hyperparameters.checkpoint_prefix}_latest",
                        type="model_checkpoint",
                        description=f"Latest model checkpoint at step {step}"
                    )
                    checkpoint_artifact.add_dir(checkpoint_path)
                    wandb.log_artifact(checkpoint_artifact, aliases=["latest"])
                
            else:
                # Save checkpoint for every evaluation step
                checkpoint_path = os.path.join(checkpoint_dir, f"{cfg.hyperparameters.checkpoint_prefix}_step_{step}")
                checkpoints.save_checkpoint(
                    ckpt_dir=checkpoint_path,
                    target=checkpoint_target,
                    step=step,
                ...
                # Upload to wandb as artifact only if needed
                if should_upload:
                    checkpoint_artifact = wandb.Artifact(
                        name=f"{cfg.hyperparameters.checkpoint_prefix}_step_{step}",
                        type="model_checkpoint",
            ...
            if len(metric_history) == 1:  # First evaluation
                logging.info(f"Checkpoints will be saved to: {checkpoint_dir}")
                logging.info(f"Save mode: {'last only' if cfg.hyperparameters.save_last_only else 'every evaluation'}")
                logging.info("Saving: critic state, actor state, actor target state, entropy coef state, KL coef state")
                if cfg.hyperparameters.checkpoint_upload_to_wandb:
                    logging.info(f"Checkpoints will be uploaded to W&B every {cfg.hyperparameters.checkpoint_upload_freq} evaluation(s)")
                else:
                    logging.info("W&B checkpoint upload is disabled")
    ...
    def load_checkpoint(checkpoint_path: str):
        """
        Load a checkpoint containing critic state, actor states, and coefficient states.
        
        Args:
            checkpoint_path: Path to the checkpoint directory
            
        Returns:
            Dictionary containing all loaded states
        """
        try:
            loaded_checkpoint = checkpoints.restore_checkpoint(checkpoint_path, target=None)
            logging.info(f"Successfully loaded checkpoint from: {checkpoint_path}")
            logging.info(f"Checkpoint contains keys: {list(loaded_checkpoint.keys())}")
            return loaded_checkpoint
        except Exception as e:
            logging.error(f"Failed to load checkpoint from {checkpoint_path}: {e}")
            raise

    # Set up the experiment: only for manipluation
