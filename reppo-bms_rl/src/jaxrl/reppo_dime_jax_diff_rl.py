...
def make_sde_eval_fn(
    ...
    def sde_evaluation_fn(
        ...
        def sde_policy(key: PRNGKey, obs: jax.Array) -> tuple[jax.Array, dict]:
            action, *_ = actor_model.sample(key, obs, stop_grad=False)
            return action, {}
    ...
    def ode_evaluation_fn(
        ...
        def ode_policy(key: PRNGKey, obs: jax.Array) -> tuple[jax.Array, dict]:
            action, *_ = actor_model.det_action(
                key, obs, stop_grad=False, ode=True, ode_coef=ode_coef
            )
            return action, {}
...
def make_train_fn(
    ...
    def learn_step(
        ...
        def update(train_state, key) -> tuple[SACTrainState, dict[str, jax.Array]]:
            def minibatch_update(carry, indices):
                ...
                output, actor_grads = actor_grad_fn(
                    train_state.actor.params
                )
                
                # Apply gradients
                actor_train_state = train_state.actor.apply_gradients(grads=actor_grads)
                train_state = train_state.replace(
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state):
            def train_step(
                carry, key: PRNGKey
            ) -> tuple[tuple[SACTrainState], tuple[dict[str, jax.Array], Transition]]:
                state = carry
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    def log_callback(state, metrics):
        ...
        log_data = {
            ...
            # log steps per second for performance monitoring
            "sps": sps,
...
@hydra.main(version_base=None, config_path="../../config", config_name="reppo_dime_jax_diff_rl")
