...
def make_train_fn(
    ...
    def update_model(
        key: PRNGKey, train_state: SACTrainState, data: tuple
        ...
        def update(carry_outer, key) -> tuple[tuple[int, SACTrainState], dict[str, jax.Array]]:
            def minibatch_update(carry_mb, indices):
                idx, train_state_mb = carry_mb
            ...
            new_carry, metrics = jax.lax.scan(
                minibatch_update, carry_outer, minibatch_idxs
            )
    ...
    jit_update_model = jax.jit(jax.vmap(update_model))
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        ...
        for i in range(num_train_steps):
            ...
            # 3. Update
            train_state, metrics = jit_update_model(jax.random.split(u_key, num_seeds), train_state, data)
            ...
            if (i + 1) % eval_interval == 0 or (i + 1) == num_train_steps:
                ...
                metrics["time/update"] = jnp.array([times_accum["update"] / eval_interval] * num_seeds)
        ...
        # Stack all metrics along the 0th dimension
        stacked_metrics = jax.tree.map(lambda *x: jnp.stack(x), *all_metrics)

        return train_state, stacked_metrics
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    def log_callback(state, metrics):
        ...
        log_msg += f" update={metrics.get('train/time/update', [0])[0]*1000:.1f}ms"
    ...
    for i in range(completed_trials, cfg.num_trials):
        ...
        sweep_metrics.append(metrics["train/episode_return"])
