        ...
        def update(train_state, key) -> tuple[SACTrainState, dict[str, jax.Array]]:
            def minibatch_update(carry, indices):
                ...
                def adjoint_matching(params):
                    ...
                    # ==========================================
                    # STEP A: Compute nabla_{a_T} Q(a_T, o)
                    # ==========================================
                    def single_sample_critic(obs, action):
                        q_val = critic_target_model.critic(obs, action)
                    ...
                    # Calculate the target vector (Gradient of Q)
                    # We use stop_gradient because we are training the actor to match this,
                    # not training the critic here.
                    Q_val, nabla_Q = batched_dq_da_fn(minibatch.critic_obs, minibatch.action)
                    Q_val, nabla_Q = jax.lax.stop_gradient(Q_val), jax.lax.stop_gradient(nabla_Q)

                    def robust_percentile_clipping(grads, max_norm):
                        ...
                        batch_p95 = jnp.percentile(sample_norms, 90.0)
                    ...
                    # Usage inside adjoint_matching:
                    tempered_nabla_Q = nabla_Q / jax.lax.stop_gradient(actor_model.temperature())
                    clipped_nabla_Q, pct_clipped = robust_percentile_clipping(tempered_nabla_Q, cfg.Q_score_max_norm)

                    # ==========================================
                    # STEP B: Diffusion / Consistency Loss
                    # ==========================================
                    ...
                    ctrl = sigma_t * jax.vmap(diffusion.fwd_model, in_axes=(0,0,0))(a_t, minibatch.obs, t)
                    old_ctrl = sigma_t * jax.vmap(old_diffusion.fwd_model, in_axes=(0,0,0))(a_t, minibatch.obs, t)

                    # Compute importance weights
                    log_importance_weights = minibatch.log_weights * jax.lax.stop_gradient(actor_model.temperature()) + Q_val.reshape(minibatch.log_weights.shape)
                    ...
                    adjoint_state = nabla_p_T_ref - clipped_nabla_Q
                    adjoint_loss = 0.5 * jnp.sum(jnp.square(ctrl + sigma_t * adjoint_state), axis=-1) * dt

                    lagrangian = actor_model.lagrangian()
                    kl_scale = jax.lax.stop_gradient(lagrangian)

                    kl_loss = 0.5 * jnp.mean(jnp.sum(jnp.square(ctrl - old_ctrl), axis=-1)) * dt

                    actor_loss = jnp.sum(self_normalized_weights.reshape(adjoint_loss.shape) * adjoint_loss) + kl_scale * jnp.mean(kl_loss)
                    # actor_loss = jnp.mean(adjoint_loss) + kl_scale * kl_loss

                    # # SAC target entropy loss
                    target_entropy = action_size_target + minibatch.log_weights
                    target_entropy_loss = (
                    ...
                    nabla_Q_norm = jnp.mean(jnp.sum(jnp.square(nabla_Q), axis=-1))
                    ...
                    metrics = dict(
                        ...
                        target_entropy=jnp.mean(action_size_target + minibatch.log_weights),
                ...
                def reverse_kl(params):
                    ...
                    # SAC actor loss
                    pred_action, log_weights = actor_model.sde_sample(key, minibatch.obs, stop_grad=False)
                    # pred_action, log_weights, kl = actor_model.sde_sample_and_kl(key, minibatch.obs, actor_target_model, stop_grad=False)
                    ...
                    # log diffusion coefficient (detached for safe logging)
                    current_log_importance_weights = log_weights * jax.lax.stop_gradient(actor_model.temperature()) + Q_val.reshape(log_weights.shape)
                    old_log_importance_weights = minibatch.log_weights * jax.lax.stop_gradient(actor_model.temperature()) + Q_val.reshape(log_weights.shape)
