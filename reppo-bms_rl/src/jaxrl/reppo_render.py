...
class ReppoConfig(struct.PyTreeNode):
    ...
    render_interval: int = 1000  # 0 means no rendering
    render_fps: int = 30
...
def make_render_fn(
    ...
    def render_fn(
        ...
        # Determine number of environments - but MJX environments are always batched
        num_envs = getattr(env, 'num_envs', 1)  # Default to 1 if not wrapped
        
        def step_env(carry, step_idx):
            key, env_state, obs = carry
            key, act_key, env_key = jax.random.split(key, 3)
            action, _ = policy(act_key, obs)
            
            # MJX environments expect batched keys
            step_key = jax.random.split(env_key, num_envs)
            
            obs, _, env_state, reward, done, info = env.step(
                step_key, env_state, action
            )
            return (key, env_state, obs), env_state

        key, init_key = jax.random.split(key)
        
        # MJX environments always expect batched keys (even for single environment)
        reset_key = jax.random.split(init_key, num_envs)
        
        # Handle different environment reset signatures
        if hasattr(env, 'reset') and norm_state is not None:
            try:
                obs, _, env_state = env.reset(reset_key, norm_state)
            except TypeError:
                # Fallback if env doesn't support norm_state parameter
                obs, _, env_state = env.reset(reset_key)
        else:
            obs, _, env_state = env.reset(reset_key)
        
        # Run rollout and collect states
        (_, _, _), trajectory_states = jax.lax.scan(
            f=step_env,
            init=(key, env_state, obs),
            xs=jnp.arange(min(max_episode_steps, 300)),  # Limit to 300 steps for memory
        )
        
        # Add initial state to trajectory  
        # trajectory_states is a State object with pytrees of arrays where first axis is time
        # We need to convert it to a list of State objects
        trajectory_list = []
        
        # Add initial state
        trajectory_list.append(env_state)
        
        # Convert trajectory_states to list - each state attribute has shape (time_steps, ...)
        # We need to extract each time step
        time_steps = jax.tree_leaves(trajectory_states)[0].shape[0]  # Get time dimension
        
        for t in range(time_steps):
            # Extract state at time t
            state_t = jax.tree_map(lambda x: x[t], trajectory_states)
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state):
            def train_step(
                ...
                return state, (metrics, transitions)

            train_key, eval_key = jax.random.split(key)
            train_state, (train_metrics, all_transitions) = jax.lax.scan(
                f=train_step,
                init=train_state,
            ...
            return train_state, metrics, all_rollout_actions

        def loop_body(
            train_state: SACTrainState, key: PRNGKey
        ) -> tuple[SACTrainState, dict]:
            key, subkey = jax.random.split(key)
            train_state, metrics, all_rollout_actions = jax.vmap(train_eval_step)(
            ...
            if action_histogram_logger is not None:
                def action_histogram_callback(state, rollout_actions):
                    ...
                    try:
                        if action_histogram_logger.verbose >= 1:
                            print(f"Found rollout actions with shape: {rollout_actions.shape}")
                        
                        # For multiple seeds, take actions from first seed
                        if len(rollout_actions.shape) > 2:  # (num_seeds, num_actions, action_dim)
                            rollout_actions = rollout_actions[0]
                            if action_histogram_logger.verbose >= 1:
                                print(f"Using first seed actions with shape: {rollout_actions.shape}")
                        
                        # Sample from all rollout actions for visualization
                        num_actions = rollout_actions.shape[0]
                        n_samples = min(action_histogram_logger.n_samples, num_actions)
                        
                        if action_histogram_logger.verbose >= 1:
                            print(f"Sampling {n_samples} actions from {num_actions} total rollout actions")
                            print(f"Rollout actions dtype: {rollout_actions.dtype}")
                            print(f"Rollout actions min/max: {rollout_actions.min():.3f}/{rollout_actions.max():.3f}")
                        
                        # Create a sampling key from the step for reproducibility
                        sample_key = jax.random.PRNGKey(step)
                        indices = jax.random.choice(sample_key, num_actions, shape=(n_samples,), replace=False)
                        sampled_actions = rollout_actions[indices]
                        
                        if action_histogram_logger.verbose >= 1:
                            print(f"Sampled actions shape: {sampled_actions.shape}")
                            print(f"Sampled actions dtype: {sampled_actions.dtype}")
                            print(f"Sample of sampled actions: {sampled_actions[:5].flatten()}")
                        
                        action_histogram_logger.log_action_histogram_from_actions(step, sampled_actions)
                    except Exception as e:
                        if action_histogram_logger.verbose >= 1:
                            print(f"Action histogram logging failed: {e}")
                ...
                jax.debug.callback(action_histogram_callback, first_train_state, first_rollout_actions)
            
            return train_state, metrics

        eval_interval = max(1, int(
...
def create_mjx_video_from_trajectory(mjx_env, trajectory_states, fps=30, width=320, height=240):
    """Create a video from MJX trajectory states."""
    try:
        # The render method expects List[State], and our trajectory_states are already State objects
        # We just need to handle the batching properly
        
        # Extract single environment states (remove batch dimensions)
        single_env_states = []
        
        for state in trajectory_states:
            # Each state should be an MJX State object - extract single env from batch
            if hasattr(state, 'obs') and hasattr(state.obs, 'shape') and state.obs.shape[0] == 1:
                # Remove batch dimension from the entire state
                single_state = jax.tree.map(lambda x: x[0] if hasattr(x, 'shape') and len(x.shape) > 0 and x.shape[0] == 1 else x, state)
                single_env_states.append(single_state)
            else:
                # Already single environment or no batch dimension
                single_env_states.append(state)
        
        # Render with MJX environment
        frames = mjx_env.render(single_env_states, height=height, width=width)
        return np.array(frames)
        
    except Exception as e:
        logging.error(f"Failed to create MJX video: {e}")
        return None
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    def log_callback(state, metrics):
        ...
        # Add video rendering for MJX environments at specified intervals
        if (cfg.env.type == "mjx" and 
            cfg.hyperparameters.get("render_interval", 0) > 0 and 
            render_fn is not None and mjx_env_for_rendering is not None and
            len(metric_history) % cfg.hyperparameters.get("render_interval", 1000) == 0):
            try:
                logging.info(f"Creating video at step {state.time_steps[0]}")
                
                # Create a simple policy for rendering
                def render_policy(key: PRNGKey, obs: jax.Array) -> tuple[jax.Array, dict]:
                    # Use the current state for deterministic actions
                    actor_model = nnx.merge(state.actor.graphdef, state.actor.params)
                    action = actor_model.det_action(obs)
                    return action, {}
                
                # Generate a trajectory for rendering
                key = jax.random.PRNGKey(42)  # Fixed seed for consistent videos
                trajectory_states = render_fn(key, render_policy, None)
                
                # Create video frames
                frames = create_mjx_video_from_trajectory(
                    mjx_env_for_rendering, 
                    trajectory_states, 
                    fps=cfg.hyperparameters.get("render_fps", 30)
                )
                
                if frames is not None and len(frames) > 0:
                    # Convert to WandB video format (T, C, H, W)
                    video_frames = frames.transpose(0, 3, 1, 2)  # (T, H, W, C) -> (T, C, H, W)
                    render_video = wandb.Video(video_frames, fps=cfg.hyperparameters.get("render_fps", 30), format="mp4")
                    log_data["render_video"] = render_video
                    logging.info(f"Successfully created video with {len(frames)} frames")
                else:
                    logging.warning("No frames generated for video")
                
            except Exception as e:
                logging.error(f"Failed to create video at step {state.time_steps[0]}: {e}")
                import traceback
                traceback.print_exc()
    ...
    # Create render function and MJX environment reference for video rendering
    mjx_env_for_rendering = None
    render_fn = None
    if cfg.env.type == "mjx" and cfg.hyperparameters.get("render_interval", 0) > 0:
        render_fn = make_render_fn(env, cfg.hyperparameters.get("render_max_steps", 1000))
        # Get reference to the underlying MJX environment
        if hasattr(env, 'env'):
            mjx_env_for_rendering = env.env
        else:
            mjx_env_for_rendering = env

    # Detect action bounds from environment
    action_bounds = None
    if hasattr(env, 'env') and hasattr(env.env, 'unwrapped'):
        if hasattr(env.env.unwrapped, 'mjx_model'):
            if hasattr(env.env.unwrapped.mjx_model, 'actuator_ctrlrange'):
                action_bounds = env.env.unwrapped.mjx_model.actuator_ctrlrange
                # Convert to numpy array (handles JAX arrays, tuples, lists)
                action_bounds = np.array(action_bounds)
                if len(action_bounds.shape) > 1 and action_bounds.shape[0] == 1:
                    action_bounds = action_bounds[0]
                print(f"MuJoCo action bounds shape: {action_bounds.shape}")
                print(f"Action bounds range: [{action_bounds.min():.3f}, {action_bounds.max():.3f}]")
