...
from src.networks.reppo_dime.jax_dime_models_pis_tanh import (
    PIS,
    DIMEActor,
)
...
def make_sde_eval_fn(
    env: Environment, max_episode_steps: int, reward_scale: float = 1.0, eval_first_episode_only: bool = False
) -> Callable[[jax.random.PRNGKey, SACTrainState, PyTreeNode | None], dict[str, float]]:
    """
    ...
    def sde_evaluation_fn(
        ...
        def step_env(carry, _):
            ...
            obs, _, env_state, reward, done, info = env.step(step_key, env_state, action, update_stats=False)
        ...
        dones = infos["returned_episode"]
        first_done_indices = jnp.argmax(dones, axis=0)
        # Mask if no env ever finished (where argmax returns 0)
        env_has_done = jnp.any(dones, axis=0)
        num_envs = dones.shape[1]

        # Compute unbiased mean/std of first episode to avoid size-bias/inspection-paradox
        def first_dones_only_agg(metric, agg_fun):
            first_metric = metric[first_done_indices, jnp.arange(num_envs)]
            return agg_fun(first_metric, where=env_has_done)

        def all_done_agg(metric, agg_fun):
            return agg_fun(metric, where=dones)

        if eval_first_episode_only:
            agg = first_dones_only_agg
        else:
            agg = all_done_agg

        return {
            # "episode_return": first_dones_only(infos["returned_episode_returns"])
            "episode_return": agg(infos["returned_episode_returns"], jnp.mean) * reward_scale,
            "episode_return_std": agg(infos["returned_episode_returns"], jnp.std),
            "episode_length": agg(infos["returned_episode_lengths"], jnp.mean),
            "episode_length_std": agg(infos["returned_episode_lengths"], jnp.std),
            "episode_success": agg(infos["returned_episode_successes"], jnp.mean),
            "episode_success_std": agg(infos["returned_episode_successes"], jnp.std),
            "num_episodes": agg(infos["returned_episode"], jnp.sum),
        }
...
def make_ode_eval_fn(
    env: Environment, max_episode_steps: int, reward_scale: float = 1.0, eval_first_episode_only: bool = False
) -> Callable[[jax.random.PRNGKey, SACTrainState, float, PyTreeNode | None], dict[str, float]]:
    """
    ...
    def ode_evaluation_fn(
        ...
        def single_sample_and_Q(key_single_sample, obs, critic_obs):
            action, *_ = actor_model.sde_sample(key_single_sample, obs, stop_grad=True)
            q_val = critic_model.critic(critic_obs, action)
            return action, q_val


        def ode_policy(key: PRNGKey, obs: jax.Array, critic_obs: jax.Array) -> tuple[jax.Array, dict]:
            sample_keys = jax.random.split(key, (int(ode_coef),))
            batched_sde_sample_and_Q = jax.vmap(single_sample_and_Q, in_axes=(0, None, None))
            actions, q_vals = batched_sde_sample_and_Q(sample_keys, obs, critic_obs)
            BoN_action = actions[jnp.argmax(q_vals, axis=0), jnp.arange(q_vals.shape[1])] 
            return BoN_action, {}
        ...
        def step_env(carry, _):
            ...
            step_key = jax.random.split(env_key, env.num_envs)
            obs, critic_obs, env_state, reward, done, info = env.step(step_key, env_state, action, update_stats=False)
...
def make_init(
    ...
    def init(key: jax.random.PRNGKey) -> SACTrainState:
        ...
        # lagrange_tx = optax.sgd(temp_lr, momentum=None)
...
def make_train_fn(
    ...
    sde_eval_fn = make_sde_eval_fn(
        eval_env, cfg.max_episode_steps, reward_scale=reward_scale, eval_first_episode_only=cfg.eval_first_episode_only
    )
    ...
    def learn_step(
        ...
        def update_actor(train_state, key) -> tuple[SACTrainState, dict[str, jax.Array]]:
            def minibatch_update(carry, indices_and_key):
                ...
                def adjoint_matching(params):
                    ...
                    # temperature = actor_model.temperature() * actor_model.fixed_temperature()
                    temperature = actor_model.temperature() #* actor_model.fixed_temperature()
                    ...
                    def clip_percentile(samples):
                        batch_p95 = jnp.percentile(samples, 99.0)

                        scale_factor = jnp.where(
                            samples > batch_p95,
                            batch_p95 / (samples + 1e-6),
                            1.0
                        )

                        return samples * scale_factor
                    # adjoint_loss = clip_percentile(adjoint_loss)
                    ...
                    # Compute importance weights
                    # log_importance_weights = log_weights.squeeze(-1) * jax.lax.stop_gradient(temperature) + Q_value
                    ...
                    match (cfg.trust_region_lagrangian, cfg.trust_region_time_weighting, cfg.trust_region_granularity):
                        ...
                        case ("dual_optimal_geometric_average_with_entropy", True, "avg"):
                            ...
                            # mean_log_pT = jnp.mean(log_p_T_ref)
                            ...
                            # Determine current entropy
                            h_old = -0.5 * norm_ui_sq_mean - mean_log_pT
                            h_target = -action_size_target # Based on your code's target entropy logic
                            
                            # Clip gamma to feasible range
                            gamma = jnp.clip(h_target, h_min, h_max)
                            # def lerp(a, b, alpha=0.5):
                            #     return (1-alpha) * a + alpha * b
                            # gamma = jnp.clip(h_target, lerp(h_old, h_min, 0.75), lerp(h_old, h_max, 0.75))

                            
                            # LBFGS
                            # dual_params, iter_num = solve_dual_2d_lbfgs(w_t, old_ctrl, ctrl_target, log_p_T_ref - cov_weight, cfg.kl_bound, gamma, init_params=jnp.concat((jnp.zeros(1), jnp.log(actor_model.fixed_temperature()))))
                            # dual_params, iter_num = solve_dual_2d_lbfgs(w_t, old_ctrl, ctrl_target, log_p_T_ref, cfg.kl_bound, gamma, init_params=jnp.concat((jnp.zeros(1), jnp.log(actor_model.fixed_temperature()))))
                            
                            # # GD + Line Search
                            # dual_params = solve_dual_2d_gd_linesearch(w_t, old_ctrl, ctrl_target, log_p_T_ref, cfg.kl_bound, gamma, init_params=jnp.concat((jnp.ones(1), actor_model.fixed_temperature())))
                            

                            # opt_lam, opt_eta = dual_params
                            # opt_eta = jnp.maximum(opt_eta, 1e-9)
                            ...
                            # ctrl_target = (w_t[:, None] * ctrl_target + lagrangian * old_ctrl) / (w_t[:, None] + lagrangian + temperature)
                            # opt_ent = (- 0.5 * jnp.sum(jnp.square(ctrl_target), axis=-1) - log_p_T_ref.squeeze()).mean()
                            # opt_kl = 0.5 * jnp.sum(jnp.square(old_ctrl - ctrl_target), axis=-1).mean()
                ...
                def update_fn(path, value):
                    # Convert the path objects to a tuple of strings for easy comparison
        ...
        def update_Q(train_state: SACTrainState, data: Transition) -> Transition:
            ...
            if cfg.Q_score_max_norm_for_squashed:
                ...
                def squash_action(action_unsquashed):
                    return distrax.Tanh().forward(action_unsquashed)
        ...
        # Already update the target critic, so we use the new one in update_Q
        train_state = train_state.replace(
    ...
    def train_fn(key: PRNGKey, cfg: ReppoConfig) -> tuple[SACTrainState, dict]:
        def train_eval_step(key, train_state):
            ...
            eval_time_steps = train_state.time_steps
            ...
            metrics = {
                ...
                "eval/time_step": eval_time_steps,
...
def run(cfg: DictConfig, trial: optuna.Trial | None) -> float:
    ...
    def log_callback(state, metrics):
        ...
        eval_success = metrics["eval/episode_success"].mean()
        
        # log_msg = f"step={state.time_steps[0]} episode_return={episode_return:.3f}, episode_length={eval_length:.3f}"
        log_msg = f"step={state.time_steps[0]} episode_return={episode_return:.3f}, episode_length={eval_length:.3f}, episode_success={eval_success:.3f}"
    ...
    if cfg.env.type == "brax":
        ...
        env = ManiSkillGymnaxWrapper(
            ...
            num_envs=cfg.hyperparameters.num_envs,
            reconfiguration_freq=None, # Training usually None
...
def compute_entropy_via_importance_sampling(log_weights, Q_value, lm_entr, CoV=None):
    ...
    # We store the CoV-aware log_weights = log dQ(X)/dP^u(X) - log q*(a) = log dQ(X)/dP^u(X) - log q*(x) + CoV
    # So simple addition of log_q_tilde i.e. q*(a) and RND gives us log dQ(X)/dP^u(X)
    ...
    #       = - E_P^u [1/Z * dQ(X)/dP^u(X) * (log q_tilde(X_T) - log dtanh(X_T)/dX_T)] + log Z
...
def parallel_nary_search(f, low, high, n_points=64, rtol=1e-4, atol=1e-6, max_iter=50):
    ...
    # Track (low, high, iteration number) as state

    def cond_fun(state):
        a, b, i = state
        # Continue refinement until max_iter or precise range
    ...
    def body_fun(state):
        ...
        # Initial range might be wrong, so expand with same high/low ratio
        ratio = high / low

        # Best is at lower bound, convexity implies that minimmum
        # either at smaller value or between two first grid points
        is_at_lower = (idx == 0)
        high_lower = grid[1]
        low_lower = high_lower / ratio

        # Best is at upper bound, convexity implies that minimmum
        # either at larger value or between two last grid points
        is_at_upper = (idx == n_points - 1)
        low_upper = grid[n_points - 2]
        high_upper = low_upper * ratio
        
        # Best is in range, convexity implies that minimmum within previous and next
        # grid points of current minimum point
        # We use jnp.maximum/minimum to handle illegal indices.
...
def solve_dual_2d_lbfgs(w_t, old_ctrl, ctrl_target, log_p_T_ref, eps, gamma, init_params=None):
    """Implementation A: Using L-BFGS to find optimal lambda and eta."""
    ...
    def dual_fun(log_params):
        # We optimize in log-space to enforce lambda >= 0, eta >= 0
        ...
        return -objective # Minimize negative to maximize

    # solver = jaxopt.LBFGS(fun=dual_fun, maxiter=200, history_size=200, tol=1e-6)
    solver = jaxopt.LBFGS(fun=dual_fun, maxiter=20, tol=1e-3)
    if init_params is None:
        # Initialize with small values
        init_params = jnp.array([0.0, 0.0]) 
    res = solver.run(init_params)
    return jnp.exp(res.params), res.state.iter_num # Return lambda, eta

def solve_dual_2d_gd_linesearch(w_t, old_ctrl, ctrl_target, log_p_T_ref, eps, gamma, init_params=None, iters=150):
    """Implementation B: Gradient Descent with Parallel Line Search."""
    dist_sq = jnp.sum(jnp.square(old_ctrl - ctrl_target), axis=-1)
    norm_ui_sq = jnp.sum(jnp.square(old_ctrl), axis=-1)
    norm_us_sq = jnp.sum(jnp.square(ctrl_target), axis=-1)
    ...
    # Candidate step sizes for parallel evaluation betwee E-9 and E+1
    step_sizes = jnp.logspace(-9, 1, 16) 

    def body_fn(p, _):
        g = grad_fn(p)
        # Try all step sizes in parallel
        candidates = p[None, :] + step_sizes[:, None] * g[None, :]
        candidates = jnp.maximum(candidates, 1e-6) # Project to non-negative
