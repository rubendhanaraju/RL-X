...
#SBATCH --array=0-7%0
#SBATCH --time=600 # in min

# uncomment from ##SBATCH to just #SBATCH depending on cluster

### Horeka
##SBATCH --output=/hkfs/work/workspace/scratch/km6079-diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.out
##SBATCH --error=/hkfs/work/workspace/scratch/km6079-diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.err
...
### HAICORE
##SBATCH --output=/hkfs/work/workspace_haic/scratch/km6079-diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.out
##SBATCH --error=/hkfs/work/workspace_haic/scratch/km6079-diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.err
...
### Juwels
#SBATCH --output=/p/home/jusers/thilges1/juwels/PROJECT_hai_1311/thilges/diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.out
#SBATCH --error=/p/home/jusers/thilges1/juwels/PROJECT_hai_1311/thilges/diffrl/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.err
...
### Kluster
##SBATCH --output=/home/i53/mitarbeiter/thilges/rl-soc/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.out
##SBATCH --error=/home/i53/mitarbeiter/thilges/rl-soc/reppo/outputs/053_mjx_hum_bench/%x_%A_%a.err
...
# SWEEP Params
# ==============================================================================
BASE_OVERRIDE="-cn reppo_pis +run=053_mjx_hum_bench/config"

# --- GRID SEARCH ---
# Define keys and then matching arrays named GRID_VALS_0, GRID_VALS_1, etc.

GRID_KEYS=("hyperparameters.diffusion.loss" "hyperparameters.diffusion.diff_steps")
GRID_VALS_0=("am" "rkl")
GRID_VALS_1=(16 128)
...
export WANDB_DIR=/p/home/jusers/thilges1/juwels/PROJECT_hai_1311/thilges/diffrl/reppo/outputs/053_mjx_hum_bench/ # for juwels
mkdir -pv $WANDB_DIR
...
for (( i=START_IDX; i<END_IDX && i<TOTAL_COUNT; i++ )); do
    ...
    # Run your command
    # HYDRA_FULL_ERROR=1 uv run src/jaxrl/reppo_pis_seperate_geom_kl_benchmark.py $CURRENT_PARAMS
    HYDRA_FULL_ERROR=1 srun --ntasks=1 --exclusive uv run src/jaxrl/reppo_pis_seperate_geom_kl_benchmark.py $CURRENT_PARAMS & # for juwels
done
