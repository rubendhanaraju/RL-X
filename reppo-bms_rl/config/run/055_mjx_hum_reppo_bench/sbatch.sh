...
#SBATCH --array=0-1%0
#SBATCH --time=120 # in min

# uncomment from ##SBATCH to just #SBATCH depending on cluster

### Horeka
##SBATCH --output=/hkfs/work/workspace/scratch/km6079-diffrl/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.out
##SBATCH --error=/hkfs/work/workspace/scratch/km6079-diffrl/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.err
...
### HAICORE
##SBATCH --output=/hkfs/work/workspace_haic/scratch/km6079-diffrl/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.out
##SBATCH --error=/hkfs/work/workspace_haic/scratch/km6079-diffrl/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.err
...
### Juwels
#SBATCH --output=/p/home/jusers/celik5/juwels/PROJECT_hai_1316/thilges/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.out
#SBATCH --error=/p/home/jusers/celik5/juwels/PROJECT_hai_1316/thilges/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.err

#SBATCH --account=hai_1316
...
### Kluster
##SBATCH --output=/home/i53/mitarbeiter/thilges/rl-soc/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.out
##SBATCH --error=/home/i53/mitarbeiter/thilges/rl-soc/reppo/outputs/055_mjx_hum_reppo_bench/%x_%A_%a.err
...
# SWEEP Params
# ==============================================================================
BASE_OVERRIDE="-cn reppo +run=055_mjx_hum_reppo_bench/config"
...
export WANDB_DIR=/p/home/jusers/celik5/juwels/PROJECT_hai_1316/thilges/reppo/outputs/055_mjx_hum_reppo_bench/ # for juwels
mkdir -pv $WANDB_DIR
...
for (( i=START_IDX; i<END_IDX && i<TOTAL_COUNT; i++ )); do
    ...
    # Run your command
    # HYDRA_FULL_ERROR=1 uv run src/jaxrl/reppo_benchmark.py $CURRENT_PARAMS
    HYDRA_FULL_ERROR=1 srun --ntasks=1 --exclusive uv run src/jaxrl/reppo_benchmark.py $CURRENT_PARAMS & # for juwels
done
