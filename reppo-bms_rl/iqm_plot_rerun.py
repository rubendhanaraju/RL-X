...
def load_data_from_csv(csv_path: str = "all_environments_data.csv") -> Dict[str, Dict]:
...
def collect_all_environments_data(entity: str, project: str, env_list: List[str], 
                                 ...
                                 grouping_config: dict = None) -> Dict[str, Dict]:
    ...
    if csv_data:
        ...
        csv_path = "all_environments_data.csv"
...
def create_multiple_3x4_subplot_grids(all_env_data: Dict[str, Dict], output_prefix: str = "environments_grid"):
    ...
    if 'reppo-sim-3' in all_algorithms:
        algorithm_colors['reppo-sim-3'] = 'C0'  # Blue
        remaining_algorithms = [algo for algo in all_algorithms if algo != 'reppo-sim-3']
    ...
    for grid_idx, env_group in enumerate(env_groups):
        ...
        for idx, env_name in enumerate(env_group):
            ...
            # iqm_scores, iqm_cis = rly_lib.get_interval_estimates(scores_dict, iqm, reps=50000)
...
def create_algorithm_group_key(algorithm_name: str, run_config: Dict, grouping_config: Dict[str, List[str]]) -> str:
    ...
    if grouping_config is None:
        ...
        grouping_config = {
            'reppo-dime-kl-c': ['kl_action_rep', 'kl_bound'],  # Group by multiple hyperparams
            'reppo-dime-9': [],  # No additional grouping
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv-path", type=str, default="all_environments_data.csv",
    ...
    grouping_config = {
        'reppo-dime-kl-c': ['kl_action_rep', 'kl_bound', 'ent_target_mult'],  # Group by KL action rep AND KL bound
        'reppo-dime-9': [],                                # No additional grouping
        'reppo-sim-3': [],                                 # No additional grouping
    ...
    if args.load_csv:
        ...
        project = "REPPO_DIME_ALL_RERUN"    # Your project name
        num_steps_list = [32]
        
        print("\n📊 Collecting all environment data from WANDB...")
        all_env_data = collect_all_environments_data(entity, project, env_list, num_steps_list, require_plot_tag=True, grouping_config=grouping_config)
    
    # Create multiple 3x4 subplot grids
    print("\n🎨 Creating multiple 3×4 subplot grids...")
    create_multiple_3x4_subplot_grids(all_env_data, "environments_grid")
