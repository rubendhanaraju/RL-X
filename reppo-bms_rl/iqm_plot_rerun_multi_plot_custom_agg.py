...
# MODE="ode_2.0"
MODE = None
ENT_TARGET_MULT_NUM = "ent2.3"
# ENT_TARGET_MULT_NUM = "ent3.5"
# ENT_TARGET_MULT_NUM = "ent5"
    ...
    for env_name in df['environment'].unique():
        ...
        for algo_name in env_data['algorithm'].unique():
            ...
            if algo_name == "reppo-sim-3":
                ...
                if ENT_TARGET_MULT_NUM in algo_name:
                    algo_data = env_data[env_data['algorithm'] == algo_name]
...
def create_multiple_3x4_subplot_grids(all_env_data: Dict[str, Dict], output_prefix: str = "environments_grid", group_rename_map: dict = None):
    ...
    for grid_idx, env_group in enumerate(env_groups):
        ...
        output_path = f"{output_prefix}_{ENT_TARGET_MULT_NUM}_{grid_idx + 1}"
...
def create_aggregated_plot(all_env_data: Dict[str, Dict], output_prefix: str = "aggregated_plot", 
                          group_rename_map: dict = None, exclude_envs: List[str] = None):
    """Create a single aggregated plot combining all environments (excluding specified ones)."""
    
    print("\n🎨 Creating aggregated plot across all environments...")
    
    if exclude_envs is None:
        exclude_envs = []
    
    # Filter environments to exclude
    env_names = [env for env in all_env_data.keys() 
                 if all_env_data[env] and env not in exclude_envs]
    print(f"   📊 Including {len(env_names)} environments (excluding: {exclude_envs})")
    
    # Get all unique algorithms across all environments
    all_algorithms = set()
    for env_name in env_names:
        env_data = all_env_data[env_name]
        for algo_name in env_data.keys():
            base_algo = '-'.join(algo_name.split('-')[:-1])
            all_algorithms.add(base_algo)
    all_algorithms = sorted(list(all_algorithms))
    
    # Aggregate data across environments for each algorithm
    aggregated_data = defaultdict(list)
    
    for env_name in env_names:
        env_data = all_env_data[env_name]
        
        for algo_name, runs in env_data.items():
            base_algo = '-'.join(algo_name.split('-')[:-1])  # Remove environment suffix
            
            # Add all runs from this environment to the aggregated data
            for steps, returns in runs:
                aggregated_data[base_algo].append((steps, returns))
    
    print(f"   📈 Aggregated data for {len(aggregated_data)} algorithms")
    
    # Prepare rliable data for the aggregated dataset
    algorithms, frames, scores_dict = prepare_rliable_data_aggregated(aggregated_data)
    ...
    algorithm_colors = {}
    if f'reppo-sim-3' in algorithms:
        algorithm_colors["reppo-sim-3"] = 'C0'  # Blue
        remaining_algorithms = [algo for algo in algorithms if algo != "reppo-sim-3"]
        for i, algo in enumerate(remaining_algorithms, start=1):
            algorithm_colors[algo] = f'C{i}'
    else:
        for i, algo in enumerate(algorithms):
    ...
    for algo in algorithms:
        ...
        # Plot IQM curve with confidence intervals
        line = ax.plot(frames, iqm_scores[algo], 
                      color=color, linewidth=2.5, label=algo)[0]
        
        # Plot confidence intervals with more visibility
        ci_lower = iqm_cis[algo][0]
        ci_upper = iqm_cis[algo][1]
        ax.fill_between(frames, ci_lower, ci_upper,
                       color=color, alpha=0.3, edgecolor=None)
        
        # Debug: Print confidence interval info
        ci_width = np.mean(ci_upper - ci_lower)
        print(f"       {algo}: CI width = {ci_width:.2f}")
        ...
        print(f"     ✓ Plotted {algo} (renamed to: {legend_label})")
    
    # Format the plot
    ax.set_xlabel('Timesteps', fontsize=14)
    ax.set_ylabel('Return', fontsize=14)
    ax.set_title(f'Aggregated Performance Across {len(env_names)} Environments ({MODE})', 
    ...
    # Add legend
    ax.legend(handles, labels, loc='best', fontsize=12, frameon=True)
    ...
    # Save plots
    output_path = f"{output_prefix}_{ENT_TARGET_MULT_NUM}_aggregated"
    plt.savefig(f"{output_path}.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_path}.pdf", bbox_inches='tight')
    
    print(f"\n     ✅ Saved aggregated plot:")
    print(f"        🖼️  {output_path}.png")
    print(f"        📄 {output_path}.pdf")
    ...
    print(f"\n✅ Created aggregated plot successfully!")


def prepare_rliable_data_aggregated(aggregated_data: Dict[str, List]) -> tuple:
    """Prepare aggregated data in rliable format for multi-environment analysis."""
    
    # Find common evaluation points across all runs and algorithms
    all_steps = set()
    for algo_name, runs in aggregated_data.items():
        for steps, returns in runs:
    ...
    # Use actual step points from the runs
    ...
    for algo in algorithms:
        runs = aggregated_data[algo]
        
        # Treat each environment as a separate "game" in rliable terminology
        # Each run becomes one environment's performance for this algorithm
        env_scores = []
        for steps, returns in runs:
            # Create mapping from steps to returns
            step_to_return = dict(zip(steps, returns))
            
            # Get returns for all common frames
            run_returns = []
            for frame in frames:
            ...
            env_scores.append(run_returns)
        
        # if env_scores:
        #     # Convert to rliable format: (1, num_envs, num_frames)
        #     # Each "environment" is actually a run from our aggregated data
        #     score_matrix = np.array(env_scores)[np.newaxis, :, :]  # (1, envs, frames)
        #     scores_dict[algo] = score_matrix
        #     print(f"   {algo}: 1 run x {score_matrix.shape[1]} environments x {score_matrix.shape[2]} frames")
        score_matrix = np.array(env_scores)
        scores_dict[algo] = score_matrix
            ...
            if len(history) > 0 and 'eval/episode_return' in history.columns:
                ...
                if 'ode-abl' in group_key:
                    ...
                    # for mode in ['sde', 'ode_0.5', 'ode_1.0', 'ode_2.0']:
                        ...
                        # grouped_data[f"{group_key}_{mode}"].append((steps, returns))
                    returns = history[f"eval/episode_return"].values
                    grouped_data[f"{group_key}"].append((steps, returns))
...
if __name__ == "__main__":
    ...
    if MODE:
        group_rename_map = {
            f"reppo-dime-kl-hist-ode-abl-ent2.3_{MODE}": f'REPPO-DIME-e2.3-{MODE}',
            f"reppo-dime-kl-hist-ode-abl-ent3.5_{MODE}": f'REPPO-DIME-e3.5-{MODE}',
            f"reppo-dime-kl-hist-ode-abl-ent5_{MODE}": f'REPPO-DIME-e5.0-{MODE}',
            f"reppo-sim-3": f'REPPO',
        }
    else:
        group_rename_map = {
            "reppo-dime-kl-hist-ode-abl-ent2.3_sde": 'REPPO-DIME-e2.3-SDE',
            "reppo-dime-kl-hist-ode-abl-ent3.5_sde": 'REPPO-DIME-e3.5-SDE',
            "reppo-dime-kl-hist-ode-abl-ent5_sde": 'REPPO-DIME-e5.0-SDE',
            "reppo-dime-kl-hist-ode-abl-ent2.3_ode_0.5": 'REPPO-DIME-e2.3-ODE-0.5',
            "reppo-dime-kl-hist-ode-abl-ent3.5_ode_0.5": 'REPPO-DIME-e3.5-ODE-0.5',
            "reppo-dime-kl-hist-ode-abl-ent5_ode_0.5": 'REPPO-DIME-e5.0-ODE-0.5',
            "reppo-dime-kl-hist-ode-abl-ent2.3_ode_1.0": 'REPPO-DIME-e2.3-ODE-1.0',
            "reppo-dime-kl-hist-ode-abl-ent3.5_ode_1.0": 'REPPO-DIME-e3.5-ODE-1.0',
            "reppo-dime-kl-hist-ode-abl-ent5_ode_1.0": 'REPPO-DIME-e5.0-ODE-1.0',
            "reppo-dime-kl-hist-ode-abl-ent2.3_ode_2.0": 'REPPO-DIME-e2.3-ODE-2.0',
            "reppo-dime-kl-hist-ode-abl-ent3.5_ode_2.0": 'REPPO-DIME-e3.5-ODE-2.0',
            "reppo-dime-kl-hist-ode-abl-ent5_ode_2.0": 'REPPO-DIME-e5.0-ODE-2.0',
            "reppo-sim-3": 'REPPO',
        }
    ...
    # Create aggregated plot (excluding PendulumSwingup)
    print("\n🎨 Creating aggregated plot across all environments...")
    # exclude_envs = ["PendulumSwingup"]  # Environments to exclude
    exclude_envs = [
        "AcrobotSwingup",
        "AcrobotSwingupSparse",
        "BallInCup",
    ...
    create_aggregated_plot(all_env_data, f"environments_aggregated_{MODE}_{TASK}", 
                          group_rename_map, exclude_envs)
